...
- Scope: all persistence and queue surfaces reachable via FastAPI canonical APIs and plugin workers.
...
| --- | --- | --- | --- | --- |
| Backend DB | `CLAWBOARD_DB_URL` (Postgres), SQLModel tables | `topic`, `task`, `logentry`, `ingestqueue`, `draft`, routing metadata | `logentry`: 90 days default | `docker compose exec -T db psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "DELETE FROM logentry WHERE to_timestamp(replace(replace(\"createdAt\", 'T', ' '), 'Z', ''), 'YYYY-MM-DD HH24:MI:SS.MS') < now() - interval '90 days';"` |
...
- Backend DB: `docker compose exec -T db psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "DELETE FROM logentry WHERE to_timestamp(replace(replace(\"createdAt\", 'T', ' '), 'Z', ''), 'YYYY-MM-DD HH24:MI:SS.MS') < now() - interval '90 days';"`
...
- Pending hardening:
  - automate plugin queue cleanup cadence,
