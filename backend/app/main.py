...
from .text_processing import (  # noqa: F401
    _normalize_label,
...
from .dispatch import (  # noqa: F401
    ...
    _openclaw_chat_dispatch_recover_stale_processing_jobs as _dispatch_recover_stale_processing_jobs,
...
def _openclaw_chat_dispatch_recover_stale_processing_jobs(now_dt: datetime) -> int:
    """Compatibility wrapper so main-module patches still affect dispatch recovery helpers."""
    previous = dispatch_module._OPENCLAW_CHAT_DISPATCH_PROCESS_STARTED_AT_ISO
    dispatch_module._OPENCLAW_CHAT_DISPATCH_PROCESS_STARTED_AT_ISO = _OPENCLAW_CHAT_DISPATCH_PROCESS_STARTED_AT_ISO
    try:
        return _dispatch_recover_stale_processing_jobs(now_dt)
    finally:
        dispatch_module._OPENCLAW_CHAT_DISPATCH_PROCESS_STARTED_AT_ISO = previous


# Override _attachments_root to use this module's ATTACHMENTS_DIR so tests
# that patch main_module.ATTACHMENTS_DIR continue to work.
def _attachments_root() -> Path:
    return Path(ATTACHMENTS_DIR)



# --- spaces: moved to .spaces module ---
...
SEARCH_LONG_QUERY_DENSE_EXPANSION_ENABLED = str(
    os.getenv("CLAWBOARD_SEARCH_LONG_QUERY_DENSE_EXPANSION_ENABLED", "1") or ""
).strip().lower() in {
    "1",
    "true",
    "yes",
...
SEARCH_LONG_QUERY_DENSE_EXPANSION_MIN_CHARS = max(
    120,
    int(os.getenv("CLAWBOARD_SEARCH_LONG_QUERY_DENSE_EXPANSION_MIN_CHARS", "220") or "220"),
)
SEARCH_LONG_QUERY_DENSE_EXPANSION_MIN_TOKENS = max(
    10,
    int(os.getenv("CLAWBOARD_SEARCH_LONG_QUERY_DENSE_EXPANSION_MIN_TOKENS", "28") or "28"),
)
SEARCH_LONG_QUERY_DENSE_EXPANSION_MAX_LOGS = max(
    0,
    int(os.getenv("CLAWBOARD_SEARCH_LONG_QUERY_DENSE_EXPANSION_MAX_LOGS", "180") or "180"),
)
...
# --- precompile: moved to .precompile module ---
...
# --- attachments: moved to .attachments module ---
...
# --- text_processing: moved to .text module ---
...
# --- resolver: moved to .resolver module ---
...
def on_startup() -> None:
    ...
    if PRECOMPILE_ENABLED and PRECOMPILE_WARM_LISTENER_ENABLED:
        _start_background_thread(target=_precompile_warm_worker, name="clawboard-precompile-warm")
    elif PRECOMPILE_ENABLED and PRECOMPILE_WARM_ON_STARTUP:
        _start_background_thread(target=_warm_precompiled_defaults, name="clawboard-precompile-startup")
    if os.getenv("CLAWBOARD_INGEST_MODE", "").lower() == "queue":
        _start_background_thread(target=_queue_worker, name="clawboard-ingest-queue-worker")
    if os.getenv("CLAWBOARD_DISABLE_SNOOZE_WORKER", "").strip() != "1":
        _start_background_thread(target=_snooze_worker, name="clawboard-snooze-worker")
    if os.getenv("CLAWBOARD_DISABLE_SESSION_ROUTING_GC", "").strip() != "1":
        _start_background_thread(target=_session_routing_gc_worker, name="clawboard-session-routing-gc")
    if os.getenv("CLAWBOARD_DISABLE_OPENCLAW_REQUEST_ROUTE_GC", "").strip() != "1":
        _start_background_thread(target=_openclaw_request_route_gc_worker, name="clawboard-openclaw-request-route-gc")
...
def on_shutdown() -> None:
    ...
    try:
        ...
        pass
    try:
        _shutdown_openclaw_assistant_log_watchdog()
    except Exception:
        logger.exception("failed to stop OpenClaw assistant-log watchdog during shutdown")
    with _BACKGROUND_THREADS_LOCK:
...
def _queue_worker() -> None:
    ...
    while not _BACKGROUND_STOP_EVENT.is_set():
        try:
            ...
            _log_background_worker_exception("clawboard-ingest-queue-worker")
        if _background_sleep(poll_interval):
            break



# --- dispatch_main: moved to .dispatch module ---


# --- dispatch_remaining: moved to .dispatch module ---
...
def _session_routing_gc_worker() -> None:
    ...
    while not _BACKGROUND_STOP_EVENT.is_set():
        try:
            ...
            _log_background_worker_exception("clawboard-session-routing-gc")
        if _background_sleep(poll_interval):
            break


def _openclaw_request_route_gc_worker() -> None:
    """Garbage-collect stale OpenClaw request-route rows."""
    poll_interval = float(os.getenv("CLAWBOARD_OPENCLAW_REQUEST_ROUTE_GC_SECONDS", str(6 * 60 * 60)))
    ttl_days = float(os.getenv("CLAWBOARD_OPENCLAW_REQUEST_ROUTE_TTL_DAYS", "90"))
    if ttl_days <= 0:
        return
    batch_size = int(os.getenv("CLAWBOARD_OPENCLAW_REQUEST_ROUTE_GC_BATCH", "1000"))
    while not _BACKGROUND_STOP_EVENT.is_set():
        try:
            cutoff_dt = datetime.now(timezone.utc) - timedelta(days=ttl_days)
            cutoff = cutoff_dt.isoformat(timespec="milliseconds").replace("+00:00", "Z")
            with get_session() as session:
                rows = (
                    session.exec(
                        select(OpenClawRequestRoute)
                        .where(OpenClawRequestRoute.updatedAt < cutoff)
                        .order_by(OpenClawRequestRoute.updatedAt.asc())
                        .limit(batch_size)
                    ).all()
                )
                if rows:
            ...
            _log_background_worker_exception("clawboard-openclaw-request-route-gc")
        if _background_sleep(poll_interval):
            break
...
def _find_by_source_identity(session, key: str):
...
def _source_identity_session_key(source_meta: dict[str, Any] | None) -> str:
    source = source_meta if isinstance(source_meta, dict) else {}
    session_key = str(source.get("sessionKey") or "").strip()
    board_key = _canonical_board_scope_session_key(source_meta=source, session_key=session_key)
    if board_key:
...
def _source_identity_timestamp_ms(created_at: str | None) -> str:
    ts = _iso_to_timestamp(created_at)
    if ts is None:
        return ""
    try:
        return str(max(0, int(round(float(ts) * 1000.0))))
    except Exception:
        return ""
...
def _source_identity_key(
    *,
    ...
    channel = _normalize_source_identity_channel(source.get("channel"))
    session_key = _source_identity_session_key(source)
    if not session_key:
        return None

    explicit_origin = ""
    for key in ("sourceIdentityKey", "originId", "originEventId", "eventId", "event_id", "uuid"):
        candidate = str(source.get(key) or "").strip()
        if candidate:
            explicit_origin = _clip(candidate, 240)
            break
    if explicit_origin:
        return f"srcid:{entry_type}:{actor}:{channel or '-'}:{session_key}:event:{explicit_origin}"

    request_id = _canonical_openclaw_request_id(str(source.get("requestId") or "").strip())
    ...
    if channel == "openclaw" and ts_ms:
        return f"srcid:{entry_type}:{actor}:{channel}:{session_key}:t:{ts_ms}"

    message_id = str(source.get("messageId") or "").strip()
    if message_id:
        normalized_message_id = (
            _openclaw_request_id_base(message_id) if message_id.lower().startswith("occhat-") else message_id
        )
        return f"srcid:{entry_type}:{actor}:{channel or '-'}:{session_key}:msg:{_clip(normalized_message_id, 240)}"
    if request_id and ts_ms:
        return f"srcid:{entry_type}:{actor}:{channel or '-'}:{session_key}:req:{request_id}:t:{ts_ms}"
    return None
...
def _openclaw_chat_cancel_log_lookback_seconds() -> int:
    """How far back cancellation scans logs for linked session keys."""
    raw = str(os.getenv("OPENCLAW_CHAT_CANCEL_LOG_LOOKBACK_SECONDS") or "").strip()
    if not raw:
        return 7 * 24 * 60 * 60
    try:
        value = int(raw)
    except Exception:
        raise RuntimeError("OPENCLAW_CHAT_CANCEL_LOG_LOOKBACK_SECONDS must be an integer")
    return max(60, min(30 * 24 * 60 * 60, value))


def _openclaw_chat_cancel_scope_lookback_seconds() -> int:
    """Short fallback window for finding active subagent sessions in the same board scope."""
    raw = str(os.getenv("OPENCLAW_CHAT_CANCEL_SCOPE_LOOKBACK_SECONDS") or "").strip()
    if not raw:
        return 20 * 60
    try:
        value = int(raw)
    except Exception:
        raise RuntimeError("OPENCLAW_CHAT_CANCEL_SCOPE_LOOKBACK_SECONDS must be an integer")
    return max(60, min(6 * 60 * 60, value))


def _openclaw_request_ids_equivalent(left: str | None, right: str | None) -> bool:
    ...
    if lhs == rhs:
        return True
    lhs_base = _openclaw_request_id_base(lhs)
    rhs_base = _openclaw_request_id_base(rhs)
    return bool(lhs_base and rhs_base and lhs_base == rhs_base)


def _canonical_openclaw_request_id(value: str | None) -> str:
    request_base = _openclaw_request_id_base(value)
    if not request_base:
        return ""
    if not request_base.lower().startswith("occhat-"):
        return ""
    return request_base


def _canonical_openclaw_request_id_from_source(source: dict[str, Any] | None) -> str:
    source_map = source if isinstance(source, dict) else {}
    request_candidate = _canonical_openclaw_request_id(str(source_map.get("requestId") or "").strip())
    if request_candidate:
        return request_candidate
    return _canonical_openclaw_request_id(str(source_map.get("messageId") or "").strip())


def _openclaw_request_id_base_from_source(
    source: dict[str, Any] | None,
    *,
    prefer_occhat: bool = True,
) -> str:
    source_map = source if isinstance(source, dict) else {}
    request_base = _openclaw_request_id_base(str(source_map.get("requestId") or "").strip())
    message_base = _openclaw_request_id_base(str(source_map.get("messageId") or "").strip())
    if prefer_occhat:
        for candidate in (request_base, message_base):
            if candidate and candidate.lower().startswith("occhat-"):
                return candidate
    return request_base or message_base or ""


def _openclaw_request_route_kind(topic_id: str | None) -> str:
    if str(topic_id or "").strip():
        return "topic"
    return "detached"


def _get_openclaw_request_route(
    session: Any,
    *,
    request_id: str | None,
) -> OpenClawRequestRoute | None:
    canonical_request_id = _canonical_openclaw_request_id(request_id)
    if not canonical_request_id:
        return None
    return session.get(OpenClawRequestRoute, canonical_request_id)


def _openclaw_request_route_scope(
    session: Any,
    *,
    route: OpenClawRequestRoute | None,
) -> tuple[str | None, str | None]:
    if route is None:
        return (None, None)
    return _validated_scope_candidate(
        session,
        topic_id=str(getattr(route, "topicId", "") or "").strip() or None,
        space_id=_normalize_space_id(getattr(route, "spaceId", None)),
    )


def _upsert_openclaw_request_route(
    session: Any,
    *,
    request_id: str | None,
...
) -> tuple[OpenClawRequestRoute | None, dict[str, Any] | None]:
    canonical_request_id = _canonical_openclaw_request_id(request_id)
    if not canonical_request_id:
        return (None, None)

    normalized_session_key = str(session_key or "").strip()
    base_session_key = _base_session_key(normalized_session_key)
    normalized_space_id = _normalize_space_id(space_id)

    resolved_space, resolved_topic = _validated_scope_candidate(
        session,
        topic_id=topic_id,
        space_id=normalized_space_id,
    )

    existing = session.get(OpenClawRequestRoute, canonical_request_id)
    now_value = now_iso()
    ...
    if existing is None:
        ...
        route_kind = _openclaw_request_route_kind(resolved_topic)
        row = OpenClawRequestRoute(
            requestId=canonical_request_id,
            sessionKey=effective_session_key,
            baseSessionKey=effective_base_session_key,
            spaceId=resolved_space,
            topicId=resolved_topic,
            routeKind=route_kind,
            routeLocked=bool(resolved_topic),
            sourceLogId=str(source_log_id or "").strip() or None,
            createdAt=now_value,
            updatedAt=now_value,
        )
        try:
            ...
            existing_race = session.get(OpenClawRequestRoute, canonical_request_id)
            if existing_race is not None:
                return (existing_race, None)
            raise
        event_data = {
            "requestId": canonical_request_id,
            "sessionKey": row.sessionKey,
            "baseSessionKey": row.baseSessionKey,
            "spaceId": row.spaceId,
            "topicId": row.topicId,
            ...
            "previousTopicId": None,
        }
        return (row, event_data)

    previous_topic_id = str(getattr(existing, "topicId", "") or "").strip() or None

    effective_space = _normalize_space_id(getattr(existing, "spaceId", None))
    effective_topic = previous_topic_id

    if resolved_topic:
        if not effective_topic:
            effective_topic = resolved_topic
            effective_space = resolved_space or effective_space
        elif resolved_topic == effective_topic:
            if resolved_space:
                effective_space = resolved_space
    ...
    if effective_topic:
        ...
        effective_session_key = base_session_key
    effective_base_session_key = _base_session_key(effective_session_key) or base_session_key or effective_session_key

    route_kind = _openclaw_request_route_kind(effective_topic)
    route_locked = bool(effective_topic)
    source_log = str(source_log_id or "").strip() or None

    changed = False
    updates = {
        "sessionKey": effective_session_key,
        "baseSessionKey": effective_base_session_key,
        "spaceId": effective_space,
        "topicId": effective_topic,
        "routeKind": route_kind,
        "routeLocked": route_locked,
        "sourceLogId": source_log or getattr(existing, "sourceLogId", None),
    }
    ...
    if changed:
        ...
        event_data = {
            ...
            "baseSessionKey": existing.baseSessionKey,
            "spaceId": existing.spaceId,
            "topicId": existing.topicId,
            "routeKind": existing.routeKind,
            "routeLocked": existing.routeLocked,
            "createdAt": existing.createdAt,
            "updatedAt": existing.updatedAt,
            "previousTopicId": previous_topic_id,
        }
    return (existing, event_data)


def _openclaw_request_id_matches_any(candidate: str | None, request_ids: Iterable[str]) -> bool:
    text = str(candidate or "").strip()
    if not text:
        return False
    for request_id in request_ids:
        if _openclaw_request_ids_equivalent(text, request_id):
            return True
    return False


def _session_key_from_log_source(entry: LogEntry) -> str:
    source = entry.source if isinstance(entry.source, dict) else {}
    return str(source.get("sessionKey") or "").strip()


def _collect_openclaw_cancel_related_sessions(
    session: Any,
    *,
    session_key: str,
    request_id_filter: str | None,
) -> tuple[list[str], list[str]]:
    primary_key = str(session_key or "").strip()
    primary_base = _base_session_key(primary_key)
    related: set[str] = set()
    if primary_key:
        related.add(primary_key)
    if primary_base:
        related.add(primary_base)

    request_ids: set[str] = set()
    if request_id_filter:
        request_ids.add(request_id_filter)

    primary_candidates = [key for key in [primary_key, primary_base] if key]
    if primary_candidates:
        session_exprs: list[Any] = []
        for candidate in primary_candidates:
            session_exprs.append(OpenClawChatDispatchQueue.sessionKey == candidate)
            session_exprs.append(OpenClawChatDispatchQueue.sessionKey.like(f"{candidate}|%"))
        open_rows = session.exec(
            select(OpenClawChatDispatchQueue).where(
                OpenClawChatDispatchQueue.status.in_(["pending", "retry", "processing"]),
                or_(*session_exprs),
            )
        ).all()
        for row in open_rows:
            row_session = str(getattr(row, "sessionKey", "") or "").strip()
            if not row_session:
                continue
            row_base = _base_session_key(row_session)
            row_request = str(getattr(row, "requestId", "") or "").strip()
            if request_id_filter and row_request and not _openclaw_request_ids_equivalent(row_request, request_id_filter):
                continue
            if row_request:
                request_ids.add(row_request)
            if row_base:
                related.add(row_base)
            else:
                related.add(row_session)

    exact_terms, like_terms = _openclaw_identifier_match_terms(list(request_ids))
    if exact_terms or like_terms:
        queue_match_exprs: list[Any] = []
        if exact_terms:
            queue_match_exprs.append(OpenClawChatDispatchQueue.requestId.in_(exact_terms))
        for like_term in like_terms:
            queue_match_exprs.append(OpenClawChatDispatchQueue.requestId.like(like_term))
        if queue_match_exprs:
            queue_rows = session.exec(
                select(OpenClawChatDispatchQueue)
                .where(or_(*queue_match_exprs))
                .order_by(OpenClawChatDispatchQueue.createdAt.desc())
                .limit(800)
            ).all()
            for row in queue_rows:
                row_request = str(getattr(row, "requestId", "") or "").strip()
                if not _openclaw_request_id_matches_any(row_request, request_ids):
                    continue
                row_session = str(getattr(row, "sessionKey", "") or "").strip()
                if not row_session:
                    continue
                row_base = _base_session_key(row_session)
                related.add(row_base or row_session)
                if row_request:
                    request_ids.add(row_request)

        now_dt = datetime.now(timezone.utc)
        cutoff_iso = _iso_after_seconds(now_dt, -float(_openclaw_chat_cancel_log_lookback_seconds()))
        log_query = select(LogEntry).where(LogEntry.createdAt >= cutoff_iso)
        if DATABASE_URL.startswith("sqlite"):
            parts: list[str] = []
            params: dict[str, Any] = {}
            for idx, identifier in enumerate(exact_terms):
                key = f"cancel_eq_{idx}"
                parts.append(
            ...
            for idx, identifier_like in enumerate(like_terms):
                key = f"cancel_like_{idx}"
                parts.append(
                    f"(json_extract(source, '$.requestId') LIKE :{key} OR json_extract(source, '$.messageId') LIKE :{key})"
                )
                params[key] = identifier_like
            if parts:
                log_query = log_query.where(text("(" + " OR ".join(parts) + ")")).params(**params)
            else:
                log_query = log_query.where(text("1=0"))
        else:
            exprs: list[Any] = []
            ...
            if exprs:
                log_query = log_query.where(or_(*exprs))
            else:
                log_query = log_query.where(text("1=0"))
        ...
        for row in log_rows:
            ...
            row_request = str(source.get("requestId") or source.get("messageId") or "").strip()
            if not _openclaw_request_id_matches_any(row_request, request_ids):
                continue
            source_session = str(source.get("sessionKey") or "").strip()
            if not source_session:
                continue
            related.add(_base_session_key(source_session) or source_session)
            if row_request:
                request_ids.add(row_request)

    board_topic_id = _parse_board_session_key(primary_base or primary_key)
    if board_topic_id:
        scope_cutoff_iso = _iso_after_seconds(
            datetime.now(timezone.utc),
            -float(_openclaw_chat_cancel_scope_lookback_seconds()),
        )
        scope_query = select(LogEntry).where(LogEntry.createdAt >= scope_cutoff_iso)
        scope_query = scope_query.where(LogEntry.topicId == board_topic_id)
        ...
        for row in scope_rows:
            source_session = _session_key_from_log_source(row)
            if not source_session:
                continue
            source_base = _base_session_key(source_session) or source_session
            if not source_base or source_base in related:
                continue
            if _is_subagent_session_key(source_base):
                related.add(source_base)

    ordered: list[str] = []
    seen: set[str] = set()
    for candidate in [primary_key, primary_base]:
    ...
    # Preserve order but normalize request ids down to base-equivalent unique values.
    request_ordered: list[str] = []
    for request_id in sorted(request_ids):
        key = str(request_id or "").strip()
        if not key:
            continue
        if any(_openclaw_request_ids_equivalent(key, existing) for existing in request_ordered):
            continue
        request_ordered.append(key)

    return (ordered, request_ordered)


def _subagent_scope_inference_lookback_seconds() -> int:
    """Bound fallback anchor lookup for subagent session scope recovery."""
    raw = str(os.getenv("CLAWBOARD_BOARD_SCOPE_SUBAGENT_TTL_HOURS") or "").strip()
    if not raw:
        return 48 * 60 * 60
    try:
        hours = int(raw)
    except Exception:
        raise RuntimeError("CLAWBOARD_BOARD_SCOPE_SUBAGENT_TTL_HOURS must be an integer")
    return max(1, min(168, hours)) * 60 * 60


def _validated_scope_candidate(
    session: Any,
    *,
    topic_id: str | None,
    space_id: str | None,
) -> tuple[str | None, str | None]:
    topic_candidate = str(topic_id or "").strip()
    space_candidate = _normalize_space_id(space_id)

    if topic_candidate:
        topic_row = session.get(Topic, topic_candidate)
        if not topic_row:
            return (None, None)
        topic_candidate = str(getattr(topic_row, "id", "") or "").strip()
        if not space_candidate:
            space_candidate = _space_id_for_topic(topic_row)

    if not topic_candidate:
        return (None, None)

    return (space_candidate, topic_candidate or None)


def _scope_from_source_metadata(
    session: Any,
    source: dict[str, Any] | None,
) -> tuple[str | None, str | None]:
    source_map = source if isinstance(source, dict) else {}
    topic_candidate = str(source_map.get("boardScopeTopicId") or source_map.get("boardScopeTaskId") or "").strip()
    space_candidate = _normalize_space_id(source_map.get("boardScopeSpaceId"))
    if not topic_candidate:
        topic_candidate = _parse_board_session_key(str(source_map.get("sessionKey") or "")) or ""
    return _validated_scope_candidate(
        session,
        topic_id=topic_candidate,
        space_id=space_candidate,
    )


def _scope_from_anchor_log_row(
    session: Any,
    row: LogEntry,
) -> tuple[str | None, str | None]:
    source_map = row.source if isinstance(row.source, dict) else {}
    topic_candidate = str(getattr(row, "topicId", "") or source_map.get("boardScopeTopicId") or source_map.get("boardScopeTaskId") or "").strip()
    space_candidate = _normalize_space_id(source_map.get("boardScopeSpaceId") or getattr(row, "spaceId", None))
    return _validated_scope_candidate(
        session,
        topic_id=topic_candidate,
        space_id=space_candidate,
...
def _session_routing_memory_lookup_keys(session_key: str) -> tuple[list[str], str | None]:
    key = str(session_key or "").strip()
    if not key:
        return ([], None)
    ...
    _add(key)
    _add(_base_session_key(key))

    parsed_topic = _parse_board_session_key(key)
    canonical_board_key: str | None = None
    if parsed_topic:
        canonical_board_key = f"clawboard:topic:{parsed_topic}"
    ...
    return (keys, canonical_board_key)


def _infer_scope_from_session_routing_memory(
    session: Any,
    *,
    session_key: str,
    ...
    best_scope: tuple[str | None, str | None] = (None, None)

    routing_candidates, canonical_board_key = _session_routing_memory_lookup_keys(session_key)
    seen_routing_keys: set[str] = set()
    for candidate in routing_candidates:
        key = str(candidate or "").strip()
        if not key or key in seen_routing_keys:
            continue
        seen_routing_keys.add(key)
        routing_row = session.get(SessionRoutingMemory, key)
        if not routing_row and "|" in key:
            routing_row = session.get(SessionRoutingMemory, key.split("|", 1)[0])
        if not routing_row or not isinstance(getattr(routing_row, "items", None), list):
            continue
        for item in reversed(list(routing_row.items or [])):
            if not isinstance(item, dict):
                continue
            candidate_topic = str(item.get("topicId") or "").strip()
            if topic_filter and candidate_topic and candidate_topic != topic_filter:
                continue
            if topic_filter and not candidate_topic:
                candidate_topic = topic_filter
            scope_space, scope_topic = _validated_scope_candidate(
                session,
                topic_id=candidate_topic or topic_filter,
                space_id=None,
                )
            if scope_topic and not best_scope[1]:
                best_scope = (scope_space, scope_topic)
    ...
    if canonical_board_key:
        like_exprs = [
            SessionRoutingMemory.sessionKey == canonical_board_key,
            SessionRoutingMemory.sessionKey.like(f"{canonical_board_key}|%"),
            SessionRoutingMemory.sessionKey.like(f"%:{canonical_board_key}"),
            SessionRoutingMemory.sessionKey.like(f"%:{canonical_board_key}|%"),
        ]
        rows = session.exec(
            select(SessionRoutingMemory)
            .where(or_(*like_exprs))
            .order_by(SessionRoutingMemory.updatedAt.desc())
            .limit(32)
        ).all()
        for routing_row in rows:
            if not isinstance(getattr(routing_row, "items", None), list):
                continue
            for item in reversed(list(routing_row.items or [])):
                if not isinstance(item, dict):
                    continue
...
def _infer_subagent_scope_from_recent_anchors(
    session: Any,
    *,
    session_key: str,
    created_at: str,
    ...
    def _bounded_query() -> Any:
        query = select(LogEntry).where(LogEntry.createdAt <= created_at)
        if lower_bound:
            query = query.where(LogEntry.createdAt >= lower_bound)
        return query

    # Primary fallback: resolve scope from parent-side handoff logs that carry the child
    # subagent session key in raw payload (sessions_spawn/clawboard_update_task, etc.).
    like_term = f"%{base_key}%"
    anchor_query = (
        _bounded_query()
        .where(LogEntry.type == "action")
        .where(or_(func.coalesce(LogEntry.raw, "").like(like_term), func.coalesce(LogEntry.content, "").like(like_term)))
        .where(LogEntry.topicId.is_not(None))
        .order_by(
            LogEntry.createdAt.desc(),
            (text("rowid DESC") if DATABASE_URL.startswith("sqlite") else LogEntry.id.desc()),
        )
    ...
    for row in anchor_rows:
        scope_space, scope_topic = _scope_from_anchor_log_row(session, row)
        if scope_topic:
            return (scope_space, scope_topic)

    # Secondary fallback: session-level continuity memory for this subagent session key.
    routing_candidates = [base_key, _base_session_key(session_key), session_key]
    seen_routing_keys: set[str] = set()
    for candidate in routing_candidates:
        key = str(candidate or "").strip()
        if not key or key in seen_routing_keys:
    ...
    # boardScope metadata (source-level, not classifier-derived assignments).
    recent_query = (
        _bounded_query()
        .where(LogEntry.type.in_(["conversation", "action"]))
        .order_by(
            LogEntry.createdAt.desc(),
            (text("rowid DESC") if DATABASE_URL.startswith("sqlite") else LogEntry.id.desc()),
        )
    ...
    for row in session.exec(recent_query).all():
        if not _log_matches_session(row, base_key):
            continue
        scope_space, scope_topic = _scope_from_source_metadata(
            session,
...
def _retro_scope_subagent_unanchored_tool_activity(
    ...
    *,
    ...
    canonical_space, canonical_topic = _validated_scope_candidate(
        session,
        topic_id=topic_id,
        space_id=space_id,
    )
    ...
    for row in rows:
        ...
        if canonical_topic:
            ...
            source_map["boardScopeKind"] = "topic"
...
def _retro_scope_request_route_rows(
    session: Any,
    *,
    request_id: str | None,
    session_key: str | None,
    ...
    if not canonical_request_id or not canonical_topic:
        return []

    candidate_session_keys = [
        str(session_key or "").strip(),
        _base_session_key(session_key),
        f"{BOARD_TOPIC_SESSION_PREFIX}{canonical_topic}",
    ]
    candidate_session_keys = list(dict.fromkeys([key for key in candidate_session_keys if key]))

    exact_terms, like_terms = _openclaw_identifier_match_terms([canonical_request_id])
    if not exact_terms and not like_terms:
        return []
    ...
    if DATABASE_URL.startswith("sqlite"):
        ...
        if not request_parts:
            return []
        query = query.where(text("(" + " OR ".join(request_parts) + ")")).params(**params)
        if candidate_session_keys:
            session_parts: list[str] = []
            session_params: dict[str, Any] = {}
            for idx, key in enumerate(candidate_session_keys):
                exact_key = f"route_session_eq_{idx}"
                like_key = f"route_session_like_{idx}"
                session_parts.append(
                    f"(json_extract(source, '$.sessionKey') = :{exact_key} OR json_extract(source, '$.sessionKey') LIKE :{like_key})"
                )
                session_params[exact_key] = key
                session_params[like_key] = f"{key}|%"
            query = query.where(text("(" + " OR ".join(session_parts) + ")")).params(**session_params)
    else:
        request_exprs: list[Any] = []
        for identifier in exact_terms:
            request_exprs.append(LogEntry.source["requestId"].as_string() == identifier)
            request_exprs.append(LogEntry.source["messageId"].as_string() == identifier)
        for identifier_like in like_terms:
            request_exprs.append(LogEntry.source["requestId"].as_string().like(identifier_like))
            request_exprs.append(LogEntry.source["messageId"].as_string().like(identifier_like))
        query = query.where(or_(*request_exprs))
        if candidate_session_keys:
            session_expr = LogEntry.source["sessionKey"].as_string()
            session_exprs: list[Any] = []
            for key in candidate_session_keys:
                session_exprs.append(session_expr == key)
                session_exprs.append(session_expr.like(f"{key}|%"))
            query = query.where(or_(*session_exprs))

    source_log_id_text = str(source_log_id or "").strip()
    if source_log_id_text:
        query = query.where(LogEntry.id != source_log_id_text)
    ...
    for row in rows:
        source_map = row.source.copy() if isinstance(row.source, dict) else {}
        current_topic = str(getattr(row, "topicId", "") or "").strip()
        current_source_topic = str(source_map.get("boardScopeTopicId") or "").strip()
        if current_topic == canonical_topic and current_source_topic == canonical_topic:
            continue
...
def _normalize_openclaw_assistant_semantic_text(value: str | None) -> str:
    cleaned = _sanitize_log_text(str(value or ""))
    if not cleaned:
        return ""
    # Collapse common markdown wrappers so plain-text and markdown variants dedupe.
...
def _assistant_conversation_payload_matches_existing(existing: LogEntry, payload: LogAppend) -> bool:
    ...
    if existing_text and payload_text:
        if existing_text == payload_text:
            return True
        existing_semantic = _normalize_openclaw_assistant_semantic_text(
            str(getattr(existing, "content", None) or getattr(existing, "summary", None) or getattr(existing, "raw", None) or "")
        )
        payload_semantic = _normalize_openclaw_assistant_semantic_text(
            str(payload.content or payload.summary or payload.raw or "")
        )
        if existing_semantic and payload_semantic:
            return existing_semantic == payload_semantic
        return False
    # If either side lacks text, preserve legacy dedupe behavior.
    return True


def _non_user_conversation_payload(payload: LogAppend) -> bool:
    if str(payload.type or "").strip().lower() != "conversation":
        return False
    agent_id = str(payload.agentId or "").strip().lower()
    return bool(agent_id) and agent_id not in {"user", "system"}


def _source_has_message_or_request_id(source_meta: dict[str, Any] | None) -> bool:
    source = source_meta if isinstance(source_meta, dict) else {}
    return bool(str(source.get("messageId") or "").strip() or str(source.get("requestId") or "").strip())


def _assistant_duplicate_markdown_score(value: str | None) -> int:
    raw = str(value or "")
    normalized = _sanitize_log_text(raw)
    if not normalized:
        return 0
...
def _maybe_upgrade_existing_duplicate_log(
    session: Any,
    *,
    existing: LogEntry,
    payload: LogAppend,
    ...
    if changed and merged_source:
        existing.source = merged_source
    elif changed and not merged_source:
        existing.source = None

    if idempotency_key and not str(getattr(existing, "idempotencyKey", None) or "").strip():
        existing.idempotencyKey = idempotency_key
        changed = True
    if source_identity_key and not str(getattr(existing, "sourceIdentityKey", None) or "").strip():
        existing.sourceIdentityKey = source_identity_key
        changed = True
...
def _find_existing_openclaw_non_user_conversation_near_duplicate(
    ...
    *,
    ...
    incoming_space_id, incoming_topic_id = incoming_scope
    has_board_scope = bool(incoming_topic_id)
    ...
    if created_ts is None:
        return None
    window_seconds = 2 * 60.0
    lower_bound = datetime.fromtimestamp(max(0.0, created_ts - window_seconds), tz=timezone.utc).isoformat(
        timespec="milliseconds"
    ).replace("+00:00", "Z")
    upper_bound = datetime.fromtimestamp(created_ts + window_seconds, tz=timezone.utc).isoformat(
        timespec="milliseconds"
    ).replace("+00:00", "Z")
    ...
    query = (
        ...
        .where(func.lower(func.coalesce(LogEntry.agentId, "")) == str(payload.agentId or "").strip().lower())
        .where(LogEntry.createdAt >= lower_bound)
        .where(LogEntry.createdAt <= upper_bound)
    )
    ...
    if DATABASE_URL.startswith("sqlite"):
        query = query.where(
            text(
                ...
                "IN ('', 'openclaw', 'clawboard', 'webchat', 'direct')"
            )
        )
    else:
    ...
    incoming_channel = str(source_meta.get("channel") or "").strip().lower()
    ...
    for candidate in candidates:
        ...
        if has_board_scope:
            candidate_space_id, candidate_topic_id = _scope_from_anchor_log_row(session, candidate)
            same_board_scope = (
...
def _canonical_board_scope_session_key(
    *,
    ...
    if isinstance(source_meta, dict):
        topic_id = str(source_meta.get("boardScopeTopicId") or source_meta.get("boardScopeTaskId") or "").strip()
        if topic_id:
            return f"clawboard:topic:{topic_id}"

    parsed_topic = _parse_board_session_key(str(session_key or ""))
    if parsed_topic:
...
def _find_existing_openclaw_user_request_log(
    ...
    *,
    ...
    request_id = _canonical_openclaw_request_id_from_source(source_meta)
    if not request_id:
        return None
...
def _find_existing_openclaw_user_request_created_at(
    ...
    *,
    ...
    base_key = _base_session_key(session_key)
    board_scope_key = _canonical_board_scope_session_key(source_meta=None, session_key=session_key)
    candidate_session_keys = [key for key in [base_key, board_scope_key] if key]
    candidate_session_keys = list(dict.fromkeys(candidate_session_keys))
    query = (
...
def _find_existing_openclaw_user_near_duplicate(
    ...
    *,
    ...
    content_text = str(payload.content or "")
    if not content_text.strip():
        return None

    channel_text = str(source_meta.get("channel") or "").strip().lower()
    if channel_text and channel_text not in {"openclaw", "clawboard", "webchat"}:
        return None
    ...
    if not candidate_session_keys:
        return None
    if not any(bool(_parse_board_session_key(key)) for key in candidate_session_keys):
        return None
    ...
    if created_ts is None:
        return None
    window_seconds = 20.0
    if channel_text == "clawboard":
        # History backfill can replay the same board user prompt tens of seconds later with
        # no request/message identifiers. Keep this wider than live hook jitter.
    ...
    query = (
        ...
        .where(LogEntry.type == "conversation")
        .where(func.lower(func.coalesce(LogEntry.agentId, "")) == "user")
        .where(LogEntry.content == content_text)
        .where(LogEntry.createdAt >= lower_bound)
        .where(LogEntry.createdAt <= upper_bound)
    )
    ...
    if DATABASE_URL.startswith("sqlite"):
        ...
        query = query.where(
            text(
                "(COALESCE(json_extract(source, '$.requestId'), '') <> '' OR "
                "COALESCE(json_extract(source, '$.messageId'), '') <> '')"
            )
        )
        query = query.where(text("lower(COALESCE(json_extract(source, '$.channel'), '')) IN ('', 'openclaw', 'clawboard', 'webchat')"))
    else:
        source_session_expr = LogEntry.source["sessionKey"].as_string()
        session_exprs = []
        for key in candidate_session_keys:
    ...
    return session.exec(query.order_by(LogEntry.createdAt.desc()).limit(1)).first()
...
def append_log_entry(session, payload: LogAppend, idempotency_key: str | None = None) -> LogEntry:
    ...
    if source_identity_key:
        existing = _find_by_source_identity(session, source_identity_key)
        if existing:
            return _maybe_upgrade_existing_duplicate_log(
                session,
                existing=existing,
    ...
    existing_user_near_duplicate = _find_existing_openclaw_user_near_duplicate(
        session,
        payload=payload,
        source_meta=source_meta,
        created_at=created_at,
    )
    if existing_user_near_duplicate:
        return existing_user_near_duplicate
    ...
    if existing_non_user_near_duplicate:
        return _maybe_upgrade_existing_duplicate_log(
            session,
            existing=existing_non_user_near_duplicate,
            payload=payload,
            source_meta=source_meta,
            updated_at=updated_at,
            idempotency_key=idempotency_key,
    ...
    request_route_seed_space_id = _normalize_space_id(payload.spaceId)
    request_route_seed_topic_id = str(payload.topicId or "").strip() or None
    scope_lock = False
    request_route_event: dict[str, Any] | None = None
    request_route_row: OpenClawRequestRoute | None = None
    board_route_retro_scoped_rows: list[LogEntry] = []
    ...
    if source_meta:
        ...
        if explicit_space:
            ...
            if not request_route_seed_space_id:
                request_route_seed_space_id = explicit_space
        session_key = str(source_meta.get("sessionKey") or "").strip()
        if session_key:
            derived_topic_id = _parse_board_session_key(session_key)
            if derived_topic_id:
                source_scope_topic_id = derived_topic_id
                scope_lock = True

        explicit_topic = str(source_meta.get("boardScopeTopicId") or source_meta.get("boardScopeTaskId") or "").strip()
        ...
        trust_explicit_topic_scope = not (_is_subagent_session_key(session_key) and explicit_topic)
        if explicit_topic and trust_explicit_topic_scope:
            source_scope_topic_id = explicit_topic
            if not request_route_seed_topic_id:
                request_route_seed_topic_id = explicit_topic

        payload_type = str(payload.type or "").strip().lower()
        payload_agent_id = str(payload.agentId or "").strip().lower()

        if session_key and _is_subagent_session_key(session_key):
            inferred_space, inferred_topic = _infer_subagent_scope_from_recent_anchors(
                session,
                session_key=session_key,
                created_at=created_at,
            )
            if inferred_space and not source_scope_space_id:
                source_scope_space_id = inferred_space
                if not space_id:
                    space_id = inferred_space
    ...
    request_route_id = _canonical_openclaw_request_id_from_source(source_meta)
    if request_route_id:
        request_route_row = _get_openclaw_request_route(session, request_id=request_route_id)
        route_space, route_topic = _openclaw_request_route_scope(session, route=request_route_row)
        if route_topic:
    ...
    # OpenClaw cron delivery/control messages should never be routed into user topics.
    ...
    # keep nested/subagent logs in the originating board Topic.
    if source_meta is not None:
    ...
    has_scope_anchor = bool(topic_id or source_scope_topic_id)
    ...
    if cron_event_filtered:
        ...
        classification_attempts = 1
        if is_memory_action:
            # Memory tool traces are non-semantic control activity and should never
            # wait on bundle scoping; classify immediately even when unanchored.
            classification_status = "classified"
            classification_error = "filtered_memory_action"
        elif has_scope_anchor:
            classification_status = "classified"
    ...
    if request_route_id:
        route_seed_space, route_seed_topic = _validated_scope_candidate(
            session,
            topic_id=request_route_seed_topic_id,
            space_id=request_route_seed_space_id or source_scope_space_id or space_id,
        )
        _route_row, request_route_event = _upsert_openclaw_request_route(
            session,
            request_id=request_route_id,
            session_key=str((source_meta or {}).get("sessionKey") or "").strip(),
            space_id=route_seed_space,
            topic_id=route_seed_topic,
    ...
    if (
        request_route_id
        and topic_id
        and request_route_event
        and (
            bool(request_route_event.get("promotion"))
            or str(request_route_event.get("previousTopicId") or "").strip() != str(topic_id or "").strip()
        )
    ):
        try:
            board_route_retro_scoped_rows = _retro_scope_request_route_rows(
                session,
                request_id=request_route_id,
                session_key=source_session_key,
                space_id=space_id,
    ...
    for scoped in retro_scoped_rows:
        ...
        if scoped_id and scoped_id in scoped_seen_ids:
            continue
        if scoped_id:
        ...
        publish_live_event(
            {
                ...
                "eventTs": str(request_route_event.get("updatedAt") or entry.updatedAt),
            }
        )
...
def _openclaw_event_session_keys(session_keys: Iterable[str]) -> list[str]:
    ordered: list[str] = []
    seen: set[str] = set()
...
def _publish_openclaw_thread_work_state(
    *,
    session_keys: Iterable[str],
    active: bool,
    request_id: str | None = None,
...
def _publish_openclaw_typing_state(
    *,
    session_keys: Iterable[str],
    typing: bool,
    request_id: str | None = None,
...
def _parse_board_session_key(session_key: str) -> str | None:
    """Parse a board session key and return the topic ID, or None."""
    key = str(session_key or "").strip()
    if not key:
        return None
...
# --- session_locks: moved to .session module ---
...
_ORCHESTRATION_RECOVERY_FOLLOW_UP_LADDER_SECONDS = [60, 120, 240, 480, 960, 1920]
_ORCHESTRATION_ITEM_STATUS_RANK = {
    "queued": 0,
    "running": 0,
    "stalled": 0,
    "done": 1,
...
def _orchestration_auto_follow_up_enabled() -> bool:
    """When True, the orchestration system auto-dispatches follow-up messages to the main agent
    when a subagent completes, ensuring results reach the user even if the main agent
    forgot to set up a cron follow-up."""
    raw = str(os.getenv("CLAWBOARD_ORCHESTRATION_AUTO_FOLLOW_UP_ENABLED") or "").strip().lower()
    if not raw:
        return True
    return raw in {"1", "true", "yes", "on"}


def _orchestration_stall_nudge_enabled() -> bool:
    """When True, the orchestration system injects a stall-recovery message to the main agent
    when main.response has been stalled for more than stall_nudge_seconds."""
    raw = str(os.getenv("CLAWBOARD_ORCHESTRATION_STALL_NUDGE_ENABLED") or "").strip().lower()
    if not raw:
        return True
    return raw in {"1", "true", "yes", "on"}


def _orchestration_stall_nudge_seconds() -> int:
    """How long main.response must be stalled before a nudge is injected (default: 5 min)."""
    raw = str(os.getenv("CLAWBOARD_ORCHESTRATION_STALL_NUDGE_SECONDS") or "").strip()
    if not raw:
        return 5 * 60
    try:
...
def _orchestration_follow_up_request_id(*, run_id: str, idempotency_suffix: str) -> str:
    idem_key = f"orchestration:follow-up:{str(run_id or '').strip()}:{str(idempotency_suffix or '').strip()}"
    return "occhat-fup-" + hashlib.sha256(idem_key.encode()).hexdigest()[:32]


def _orchestration_enqueue_follow_up_dispatch(
    *,
    run: Any,
    message: str,
    ...
    if not _openclaw_chat_dispatch_enabled():
        return False
    session_key = str(getattr(run, "baseSessionKey", "") or "").strip()
    if not session_key:
        return False
    ...
    request_id = _orchestration_follow_up_request_id(run_id=run_id, idempotency_suffix=idempotency_suffix)
    try:
        _enqueue_openclaw_chat_dispatch(
            request_id=request_id,
            session_key=session_key,
...
def _orchestration_poll_seconds() -> float:
    ...
    try:
        ...
        return 5.0
    clamped = max(2.0, min(300.0, value))
    if clamped != value:
        _warn_env_clamp_once(
            "CLAWBOARD_ORCHESTRATION_POLL_SECONDS",
            raw,
            clamped=clamped,
            min_value=2.0,
            max_value=300.0,
...
def _orchestration_should_emit_in_band_event(
    *,
    ...
    if event_type == "item_created" and str(item_key or "").strip().lower() == "main.response":
        return False
    if event_type == "item_check_in":
        # Keep narration focused on the primary/main delegate loop.
        return str(item_key or "").strip().lower() == "main.response"
...
def _orchestration_event_summary_and_content(
    *,
    ...
    if event_type == "item_check_in":
        attempt = max(1, int(details.get("attempt") or 0))
        active_items = max(0, int(details.get("activeItems") or 0))
        next_check_seconds = max(0, int(details.get("nextCheckInSeconds") or 0))
        waiting = details.get("waitingItemKeys")
        ...
        if isinstance(waiting, list):
            for value in waiting:
                ...
                if len(waiting_keys) >= 3:
                    break
        summary = f"Main agent check-in #{attempt}"
        bits = [f"Monitoring active work: {active_items} delegated item(s) still running."]
        if waiting_keys:
            bits.append(f"Waiting on: {', '.join(waiting_keys)}.")
        if next_check_seconds > 0:
            bits.append(f"Next check in ~{next_check_seconds}s.")
...
def _orchestration_append_in_band_log(
    ...
    *,
    ...
    if not board_topic_scope:
        board_topic_scope = _parse_board_session_key(str(getattr(run, "baseSessionKey", "") or getattr(run, "sessionKey", "") or "")) or ""
    # In-band orchestration logs are for board-origin runs only.
    if not board_topic_scope:
        return
    board_topic_row = session.get(Topic, board_topic_scope)
    if board_topic_row is None:
        # Stale orchestration runs can outlive their original board topic. Skip the in-band
        # board log rather than writing a system log with a broken foreign key.
        return
    ...
    if not run_space_id:
        run_space_id = _space_id_for_topic(board_topic_row)
    if not run_space_id:
    ...
    if run_status:
        source["runStatus"] = run_status
        source["runActive"] = run_status not in {"done", "failed", "cancelled"}
    ...
    session.add(
        LogEntry(
            ...
            topicId=board_topic_scope or None,
            relatedLogId=None,
...
def _orchestration_sessions_spawn_result_status(entry: LogEntry) -> str | None:
    if str(getattr(entry, "type", "") or "").strip().lower() != "action":
        return None
    combined = _combined_log_text(
        _safe_log_attr_text(entry, "content"),
    ...
    if "sessions_spawn" not in combined:
        return None
    if "tool result:" not in combined and "tool error:" not in combined:
        return None

    raw_text = _safe_log_attr_text(entry, "raw").strip()
    if not raw_text:
        return None
    ...
    if isinstance(details, dict):
        if details.get("isError") is True:
            candidates.append("error")
        if str(details.get("error") or "").strip():
            candidates.append("error")
        candidates.append(details.get("status"))
    result = parsed.get("result")
    if isinstance(result, dict):
        result_details = result.get("details")
        if isinstance(result_details, dict):
            if result_details.get("isError") is True:
                candidates.append("error")
            if str(result_details.get("error") or "").strip():
                candidates.append("error")
            candidates.append(result_details.get("status"))
        if result.get("isError") is True:
            candidates.append("error")
        if str(result.get("error") or "").strip():
            candidates.append("error")
        candidates.append(result.get("status"))
        content_rows = result.get("content")
        if isinstance(content_rows, list):
            for row in content_rows:
                if not isinstance(row, dict):
                    continue
                text_payload = str(row.get("text") or "").strip()
                if not text_payload:
                    continue
                try:
                ...
                if isinstance(nested, dict):
                    if nested.get("isError") is True:
                        candidates.append("error")
                    if str(nested.get("error") or "").strip():
                        candidates.append("error")
    ...
    if str(parsed.get("error") or "").strip():
        candidates.append("error")
    candidates.append(parsed.get("status"))
...
def _orchestration_ensure_main_item(session: Any, run: OrchestrationRun, *, now_value: str) -> OrchestrationItem:
    ...
    if existing is not None:
        changed = False
        if not normalize_iso(str(getattr(existing, "startedAt", "") or "")):
            fallback_started = normalize_iso(str(getattr(run, "startedAt", "") or "")) or now_value
            existing.startedAt = fallback_started
            changed = True
        ...
        if not last_activity:
            last_activity = (
                normalize_iso(str(getattr(existing, "startedAt", "") or ""))
                or normalize_iso(str(getattr(existing, "createdAt", "") or ""))
                or normalize_iso(str(getattr(run, "startedAt", "") or ""))
            )
            if last_activity:
                meta["lastActivityAt"] = last_activity
                changed = True

        if (
            str(getattr(existing, "status", "") or "").strip().lower() not in _ORCHESTRATION_TERMINAL_ITEM_STATUSES
            and not normalize_iso(str(getattr(existing, "nextCheckAt", "") or ""))
        ):
            existing.nextCheckAt = _orchestration_next_check_iso(attempts=int(getattr(existing, "attempts", 0) or 0))
            changed = True
...
def _orchestration_create_or_get_run(
    ...
    *,
    ...
    if not request_base:
        return None
    resolved_topic_id, _ = _orchestration_resolve_scope(
        session_key=session_key,
        topic_id=topic_id,
    )
    ...
    if existing is not None:
        ...
        if changed:
            ...
            if str(getattr(existing, "topicId", "") or "").strip():
                _orchestration_append_in_band_log(
                    session,
...
def _orchestration_active_subagent_items(items: list["OrchestrationItem"]) -> list["OrchestrationItem"]:
    active: list["OrchestrationItem"] = []
    for item in items:
...
def _orchestration_has_blocking_subagent_failures(items: list["OrchestrationItem"]) -> bool:
    for item in items:
        if str(getattr(item, "kind", "") or "").strip().lower() != "subagent":
            continue
        if str(getattr(item, "status", "") or "").strip().lower() in {"failed", "cancelled"}:
            return True
    return False


def _orchestration_terminal_failed_subagent_items(items: list["OrchestrationItem"]) -> list["OrchestrationItem"]:
    failed_items: list["OrchestrationItem"] = []
    for item in items:
        if str(getattr(item, "kind", "") or "").strip().lower() != "subagent":
            continue
        if str(getattr(item, "status", "") or "").strip().lower() not in {"failed", "cancelled"}:
            continue
        failed_items.append(item)
    return failed_items


def _orchestration_latest_failed_subagent_iso(items: list["OrchestrationItem"]) -> str | None:
    latest_iso: str | None = None
    for item in _orchestration_terminal_failed_subagent_items(items):
        candidate_iso = (
...
def _orchestration_follow_up_request_pending(session: Any, request_id: str | None) -> bool:
    rid = str(request_id or "").strip()
    if not rid:
        return False
    return bool(
        session.exec(
            select(OpenClawChatDispatchQueue.id)
            .where(OpenClawChatDispatchQueue.requestId == rid)
            .where(OpenClawChatDispatchQueue.status.in_(["pending", "processing", "retry"]))
            .limit(1)
        ).first()
        is not None
    )
...
def _orchestration_recovery_follow_up_delay_seconds(attempts: int) -> float:
    index = max(0, min(len(_ORCHESTRATION_RECOVERY_FOLLOW_UP_LADDER_SECONDS) - 1, int(attempts or 0)))
    return float(_ORCHESTRATION_RECOVERY_FOLLOW_UP_LADDER_SECONDS[index])


def _orchestration_recovery_meta(run: "OrchestrationRun") -> dict[str, Any]:
...
def _orchestration_store_recovery_meta(
    session: Any,
    *,
    run: "OrchestrationRun",
    ...
    current_meta = dict(getattr(run, "meta", None) or {})
    next_meta = dict(current_meta)
    if recovery:
        next_meta["recovery"] = dict(recovery)
    else:
        next_meta.pop("recovery", None)
...
def _orchestration_main_last_activity_iso(main_item: "OrchestrationItem" | None, run: "OrchestrationRun") -> str | None:
    if main_item is None:
        return normalize_iso(str(getattr(run, "updatedAt", "") or "")) or normalize_iso(str(getattr(run, "startedAt", "") or ""))
    meta = dict(getattr(main_item, "meta", None) or {})
    return (
        normalize_iso(str(meta.get("lastActivityAt") or ""))
        or normalize_iso(str(getattr(main_item, "updatedAt", "") or ""))
        or normalize_iso(str(getattr(main_item, "startedAt", "") or ""))
        or normalize_iso(str(getattr(run, "updatedAt", "") or ""))
        or normalize_iso(str(getattr(run, "startedAt", "") or ""))
    )


def _orchestration_reopen_item(
    session: Any,
    *,
    run_id: str,
    item: "OrchestrationItem",
    ...
    if current not in _ORCHESTRATION_TERMINAL_ITEM_STATUSES:
        return False
    if current == "cancelled":
        return False
    item.status = "running"
    item.completedAt = None
    item.nextCheckAt = _orchestration_next_check_iso(attempts=0)
    item.attempts = 0
    item.lastError = None
    item.updatedAt = now_value
    meta = dict(item.meta or {})
    meta["lastActivityAt"] = now_value
    meta["reopenedAt"] = now_value
    if reason:
        meta["reopenedReason"] = reason
    meta.pop("lastCheckedAt", None)
    item.meta = meta
    session.add(item)
    _orchestration_append_event(
        session,
        ...
        payload={"from": current, "to": "running", "reason": reason},
        created_at=now_value,
    )
    return True
...
def _orchestration_recovery_follow_up_message(items: list["OrchestrationItem"]) -> str:
    failed_items = _orchestration_terminal_failed_subagent_items(items)
    if not failed_items:
        return (
            "[ORCHESTRATION_RECOVERY] Delegated work needs recovery. Please run CLAWBOARD LEDGER RECOVERY, "
            "check sessions_list, inspect the latest delegated session output, and then update the user in this thread."
        )
    ...
    for item in failed_items[:3]:
        ...
        status = str(getattr(item, "status", "") or "").strip().lower() or "failed"
        detail = _clip(_sanitize_log_text(str(getattr(item, "lastError", "") or "")), 180)
        segment = f"{agent_id} ({item_session}) -> {status}"
        if detail:
            segment += f": {detail}"
        lines.append(segment)
    if len(failed_items) > 3:
        lines.append(f"+{len(failed_items) - 3} more failed delegated attempt(s)")
    joined = "; ".join(lines)
    return (
...
def _orchestration_maybe_enqueue_recovery_follow_up(
    session: Any,
    *,
    run: "OrchestrationRun",
    items: list["OrchestrationItem"],
    ...
    recovery = _orchestration_recovery_meta(run)
    last_request_id = str(recovery.get("lastRequestId") or "").strip()
    if _orchestration_follow_up_request_pending(session, last_request_id):
        return False

    last_requested_at = normalize_iso(str(recovery.get("lastRequestedAt") or ""))
    last_main_activity = _orchestration_main_last_activity_iso(main_item, run)
    latest_failure_at = _orchestration_latest_failed_subagent_iso(items)
    attempts = max(0, int(recovery.get("attempts") or 0))
    if not force and last_requested_at:
        baseline = max(
            [candidate for candidate in [last_requested_at, last_main_activity, latest_failure_at] if candidate],
            default=last_requested_at,
        )
        baseline_ts = _iso_to_timestamp(baseline)
        if baseline_ts is not None:
            delay_seconds = _orchestration_recovery_follow_up_delay_seconds(attempts)
            if _iso_to_timestamp(now_value) is not None and (_iso_to_timestamp(now_value) - baseline_ts) < delay_seconds:
                return False

    next_attempt = attempts + 1
    idempotency_suffix = f"recovery:{next_attempt}"
    request_id = _orchestration_follow_up_request_id(run_id=run.runId, idempotency_suffix=idempotency_suffix)
    enqueued = _orchestration_enqueue_follow_up_dispatch(
        run=run,
    ...
    next_recovery = dict(recovery)
    next_recovery["attempts"] = next_attempt
    next_recovery["lastRequestedAt"] = now_value
    next_recovery["lastRequestId"] = request_id
    next_recovery["lastTrigger"] = str(trigger or "").strip() or "recovery"
    next_recovery["lastFailureAt"] = latest_failure_at or now_value
    _orchestration_store_recovery_meta(session, run=run, recovery=next_recovery, now_value=now_value)
    return True


def _orchestration_enqueue_subagent_failure_follow_up(
    session: Any,
    *,
    run: "OrchestrationRun",
    items: list["OrchestrationItem"],
    item: "OrchestrationItem",
    main_item: "OrchestrationItem" | None,
    status: str,
    detail: str,
    now_value: str,
) -> bool:
    ...
    if str(getattr(main_item, "status", "") or "").strip().lower() in _ORCHESTRATION_TERMINAL_ITEM_STATUSES:
        return False
    if str(getattr(run, "status", "") or "").strip().lower() == "cancelled":
        return False
    if str(status or "").strip().lower() == "cancelled" and "user_cancelled" in str(detail or "").strip().lower():
        return False
    return _orchestration_maybe_enqueue_recovery_follow_up(
        session,
        run=run,
        items=items,
        now_value=now_value,
        trigger=f"subagent_{str(status or '').strip().lower() or 'failed'}",
        force=True,
    )
...
def _orchestration_mark_item_status(
    ...
    *,
    ...
    if current == next_status:
        return False
    current_terminal = current in _ORCHESTRATION_TERMINAL_ITEM_STATUSES
    next_terminal = next_status in _ORCHESTRATION_TERMINAL_ITEM_STATUSES
    if current_terminal and not next_terminal:
        return False
    if current_terminal and next_terminal:
...
def _orchestration_recompute_run_status(
    ...
    *,
    ...
    if active_items:
        ...
        main_status = str(getattr(main_item, "status", "") or "").strip().lower()
    ...
    if next_status in {"done", "failed", "cancelled"} or not _orchestration_has_blocking_subagent_failures(items):
        _orchestration_store_recovery_meta(session, run=run, recovery=None, now_value=now_value)
...
def _orchestration_resolve_run_for_log(session: Any, *, request_id: str | None, session_key: str | None) -> OrchestrationRun | None:
    ...
    if not session_base:
        return None
    candidate_session_keys, canonical_board_key = _session_routing_memory_lookup_keys(session_base)
    if not candidate_session_keys:
        candidate_session_keys = [session_base]
    item = session.exec(
        select(OrchestrationItem)
        .where(OrchestrationItem.sessionKey.in_(candidate_session_keys))
    ...
    if item is None:
        if canonical_board_key:
            return session.exec(
                ...
                .where(OrchestrationRun.baseSessionKey == canonical_board_key)
    ...
    *,
    ...
    if len(ordered_items) > 3:
        specialist_segments.append(
            f"+{len(ordered_items) - 3} more completed specialist result(s) already visible in the current topic thread."
        )
    ...
    return (
        ...
        f"{'their results are' if plural else 'the result is'} already visible in the current topic thread. "
        "Read the injected current topic thread before replying. Do not re-dispatch specialists that already spawned or completed for this topic. "
        ...
        "Stay on the current topic/session and avoid broad recovery or unrelated search. "
...
def _orchestration_sync_log(log_id: str) -> None:
    ...
    try:
        with get_session() as session:
            ...
            spawn_result_status = _orchestration_sessions_spawn_result_status(entry)

            if _orchestration_is_sessions_spawn_action(entry) and (
                request_matches_run or _session_keys_equivalent(source_session, run.baseSessionKey)
            ):
                if spawn_result_status in {"error", "failed", "aborted", "cancelled"}:
                    children = []
                else:
            ...
            if source_session:
                for item in items:
                    ...
                    if item_session and _session_keys_equivalent(item_session, source_session):
            ...
            if request_terminal:
                ...
                for item in targets:
                    ...
                    if (
                        changed
                        and str(getattr(item, "kind", "") or "").strip().lower() == "subagent"
                        and terminal_status in {"failed", "cancelled"}
                    ):
                        _orchestration_enqueue_subagent_failure_follow_up(
                            session,
                            run=run,
                            items=items,
                            item=item,
            ...
            _orchestration_recompute_run_status(session, run=run, items=final_items, now_value=now_value)
            session.commit()
    except Exception:
...
def _orchestration_tick_once(*, now_dt: datetime | None = None) -> dict[str, int]:
    ...
    with get_session() as session:
        ...
        repair_cutoff_iso = _iso_after_seconds(current_dt, -float(_orchestration_recovery_lookback_seconds()))
        legacy_terminal_runs = session.exec(
            select(OrchestrationRun)
            .where(OrchestrationRun.updatedAt >= repair_cutoff_iso)
            .where(OrchestrationRun.status.in_(["done", "failed"]))
        ...
        for legacy_run in legacy_terminal_runs:
            legacy_items = session.exec(select(OrchestrationItem).where(OrchestrationItem.runId == legacy_run.runId)).all()
            if not legacy_items:
                continue
            main_resp = _orchestration_main_item(legacy_items)
            if main_resp is None:
                continue
            main_status = str(getattr(main_resp, "status", "") or "").strip().lower()
            needs_supervision = (
            ...
            refreshed_items = session.exec(select(OrchestrationItem).where(OrchestrationItem.runId == legacy_run.runId)).all()
            _orchestration_maybe_enqueue_recovery_follow_up(
                session,
                run=legacy_run,
                items=refreshed_items,
                ...
                force=not bool(_orchestration_active_subagent_items(refreshed_items)),
            )
            _orchestration_recompute_run_status(session, run=legacy_run, items=refreshed_items, now_value=current_iso)
            recovered_runs += 1
        ...
        for run in active_rows:
            ...
            for item in items:
                    ...
                    if str(getattr(item, "itemKey", "") or "").strip().lower() == "main.response":
                        ...
                        for candidate in items:
                            ...
                            if candidate_status in _ORCHESTRATION_TERMINAL_ITEM_STATUSES:
                                continue
                            candidate_key = str(getattr(candidate, "itemKey", "") or "").strip()
                ...
                if not last_activity:
                    fallback_activity = (
                        normalize_iso(str(getattr(item, "startedAt", "") or ""))
                        or normalize_iso(str(getattr(item, "createdAt", "") or ""))
                        or normalize_iso(str(getattr(run, "startedAt", "") or ""))
                    )
                    if fallback_activity:
                        ...
                        updated_meta["lastActivityAt"] = fallback_activity
                        meta = updated_meta
                ...
                if idle_seconds >= float(_orchestration_stall_seconds()):
                    if status != "stalled":
                        ...
                        is_main_item = str(getattr(item, "itemKey", "") or "").strip().lower() == "main.response"
                        if is_main_item and _orchestration_stall_nudge_enabled() and idle_seconds >= float(_orchestration_stall_nudge_seconds()):
                            waiting_keys = [
                                str(getattr(it, "itemKey", "") or "")
                                for it in items
                                if str(getattr(it, "status", "") or "").strip().lower() not in _ORCHESTRATION_TERMINAL_ITEM_STATUSES
                                and str(getattr(it, "itemKey", "") or "").strip().lower() != "main.response"
                            ]
                            waiting_summary = (", ".join(waiting_keys[:4]) or "none") if waiting_keys else "none"
                            nudge_msg = (
                                f"[ORCHESTRATION_STALL_RECOVERY] The system detected that the main agent "
                                f"has been idle for {int(idle_seconds)}s on run {run.runId}. "
                                f"Active subagent items: {waiting_summary}. "
                                f"Please run CLAWBOARD LEDGER RECOVERY: check sessions_list, "
                                f"relay any completed subagent results to the user, and confirm status."
                            )
...
def _orchestration_worker() -> None:
    ...
    while not _BACKGROUND_STOP_EVENT.is_set():
        try:
            ...
            _log_background_worker_exception("clawboard-orchestration-worker")
    ...
    *,
    ...
    def _run_openclaw_chat_via_openresponses(
        ...
        *,
        ...
        content_parts: list[dict[str, Any]] = [{"type": "input_text", "text": dispatch_message}]
    ...
    if dispatch_attempt_num > 1:
        dispatch_retry_suffix = f"retry-{dispatch_attempt_num - 1}"
    board_topic_scope_id = _parse_board_session_key(session_key)
    is_board_session = bool(board_topic_scope_id)
    ...
    send_params: dict[str, Any] = {
        ...
        # Durable dispatch retries must not reuse the exact same idempotency key forever,
        # or gateway-side cache can keep replaying a stale run response.
        "idempotencyKey": _openclaw_chat_request_id_with_suffix(request_id, suffix=dispatch_retry_suffix),
    }
    send_timeout_seconds = _openclaw_chat_send_rpc_timeout_seconds()
    lock_key, lock = _openclaw_chat_acquire_session_lock(session_key)
    lock.acquire()
    ...
    def _publish_typing_stop() -> None:
        ...
        if typing_stopped:
            return
        _publish_openclaw_typing_state(
            session_keys=[session_key],
            typing=False,
            request_id=request_id,
        )
    ...
    try:
        ...
        if prepared_attachments:
            try:
                _ensure_openclaw_attachment_model_for_dispatch(
                    session_key=session_key,
                    agent_id=agent_id,
                    attachments=prepared_attachments,
                )
                stage_info = _stage_openclaw_chat_attachments_for_agents(
                    request_id=request_id,
                    session_key=session_key,
                    attachments=prepared_attachments,
                )
                dispatch_message = f"{message}\n\n{_build_openclaw_attachment_handoff_note(stage_info)}"
            except Exception as exc:
                raw = str(exc or "")
        ...
        used_board_probe_fallback = False
        if probe_seconds <= 0:
            if is_board_session:
                # Board topic sessions should not sit idle waiting for long-stall watchdog paths.
                probe_seconds = _openclaw_chat_board_in_flight_probe_seconds()
                used_board_probe_fallback = probe_seconds > 0
        ...
        _openclaw_chat_release_session_lock(lock_key, lock)
...
def _openclaw_chat_request_has_non_user_activity(
    *,
    ...
    topic_id = _parse_board_session_key(base_key)
...
def _openclaw_chat_board_in_flight_probe_seconds() -> float:
...
def _openclaw_chat_task_in_flight_probe_seconds() -> float:
    """Backward-compatible alias for board-session fallback probe."""
    return _openclaw_chat_board_in_flight_probe_seconds()


def _openclaw_chat_board_in_flight_retry_grace_seconds() -> float:
    """Board-session retry grace cap to keep chat recovery responsive."""
    raw = str(os.getenv("OPENCLAW_CHAT_BOARD_IN_FLIGHT_RETRY_GRACE_SECONDS") or "").strip()
    if not raw:
        raw = str(os.getenv("OPENCLAW_CHAT_TASK_IN_FLIGHT_RETRY_GRACE_SECONDS") or "30").strip()
    try:
        value = float(raw)
    except Exception:
        value = 30.0
...
def _openclaw_chat_board_in_flight_abort_retry_enabled() -> bool:
    """Enable legacy abort+retry recovery for board sessions after no-progress probes."""
    raw = str(os.getenv("OPENCLAW_CHAT_BOARD_IN_FLIGHT_ABORT_RETRY") or "").strip()
    if not raw:
        raw = str(os.getenv("OPENCLAW_CHAT_TASK_IN_FLIGHT_ABORT_RETRY") or "0").strip()
    return raw.lower() in {"1", "true", "yes", "on"}


def _openclaw_chat_board_in_flight_direct_retry_enabled() -> bool:
    """Retry board-session chat.send once without abort when no-progress probe expires."""
    raw = str(os.getenv("OPENCLAW_CHAT_BOARD_IN_FLIGHT_DIRECT_RETRY") or "").strip()
    if not raw:
        raw = str(os.getenv("OPENCLAW_CHAT_TASK_IN_FLIGHT_DIRECT_RETRY") or "1").strip()
    return raw.lower() in {"1", "true", "yes", "on"}
...
def _openclaw_chat_assistant_log_backfill_base_seconds() -> float:
    """Initial cooldown between watchdog-triggered single-session history backfills."""
    raw = os.getenv("OPENCLAW_CHAT_ASSISTANT_LOG_BACKFILL_BASE_SECONDS", "60").strip()
    try:
        value = float(raw)
    except Exception:
        value = 60.0
...
def _openclaw_chat_assistant_log_backfill_max_seconds() -> float:
    """Maximum watchdog history-backfill cooldown per session."""
    raw = os.getenv("OPENCLAW_CHAT_ASSISTANT_LOG_BACKFILL_MAX_SECONDS", "3600").strip()
    try:
        value = float(raw)
    except Exception:
        value = 3600.0
...
def _openclaw_chat_assistant_log_backfill_state_ttl_seconds() -> float:
    """How long to retain per-session watchdog backfill cooldown state."""
    raw = os.getenv("OPENCLAW_CHAT_ASSISTANT_LOG_BACKFILL_STATE_TTL_SECONDS", "86400").strip()
    try:
        value = float(raw)
    except Exception:
        value = 86400.0
...
def _openclaw_gateway_history_sync_rpc_timeout_seconds() -> float:
    ...
    raw = os.getenv("OPENCLAW_GATEWAY_HISTORY_SYNC_RPC_TIMEOUT_SECONDS", "15").strip()
    try:
        value = float(raw)
    except Exception:
        value = 15.0
...
def _openclaw_gateway_history_sync_subagent_seed_limit() -> int:
    """How many delegated subagent sessions to seed each cycle from sessions_spawn lineage."""
    raw = os.getenv("OPENCLAW_GATEWAY_HISTORY_SYNC_SUBAGENT_SEED_LIMIT", "16").strip()
    try:
        value = int(raw)
    except Exception:
        value = 16
...
def _openclaw_gateway_history_sync_subagent_seed_lookback_seconds() -> int:
    """Lookback window for delegated subagent fallback seed detection."""
    raw = os.getenv("OPENCLAW_GATEWAY_HISTORY_SYNC_SUBAGENT_SEED_LOOKBACK_SECONDS", "604800").strip()
    try:
        value = int(raw)
    except Exception:
        value = 604800
...
def _openclaw_gateway_history_sync_session_backoff_base_seconds() -> float:
    raw = os.getenv("OPENCLAW_GATEWAY_HISTORY_SYNC_SESSION_BACKOFF_BASE_SECONDS", "30").strip()
    try:
        value = float(raw)
    except Exception:
        value = 30.0
...
def _openclaw_gateway_history_sync_session_backoff_ttl_seconds() -> float:
    raw = os.getenv("OPENCLAW_GATEWAY_HISTORY_SYNC_SESSION_BACKOFF_TTL_SECONDS", "86400").strip()
    try:
        value = float(raw)
    except Exception:
        value = 86400.0
...
def _openclaw_gateway_history_sync_transport_retry_attempts() -> int:
    raw = os.getenv("OPENCLAW_GATEWAY_HISTORY_SYNC_TRANSPORT_RETRY_ATTEMPTS", "2").strip()
    try:
        value = int(raw)
    except Exception:
        value = 2
...
def _openclaw_gateway_history_sync_transport_retry_base_seconds() -> float:
    raw = os.getenv("OPENCLAW_GATEWAY_HISTORY_SYNC_TRANSPORT_RETRY_BASE_SECONDS", "1.5").strip()
    try:
        value = float(raw)
    except Exception:
        value = 1.5
...
_OPENCLAW_WATCHDOG_BACKFILL_STATE: dict[str, dict[str, Any]] = {}


def _openclaw_watchdog_backfill_delay_seconds(streak: int) -> float:
    base = _openclaw_chat_assistant_log_backfill_base_seconds()
    max_delay = _openclaw_chat_assistant_log_backfill_max_seconds()
    exponent = max(0, min(10, int(streak or 0) - 1))
    delay = base * float(2**exponent)
    return max(base, min(max_delay, delay))


def _openclaw_watchdog_backfill_should_run(session_key: str, *, now_mono: float | None = None) -> bool:
    key = str(session_key or "").strip()
    if not key:
        return False
    current = time.monotonic() if now_mono is None else float(now_mono)
    ttl_seconds = _openclaw_chat_assistant_log_backfill_state_ttl_seconds()
    with _OPENCLAW_WATCHDOG_BACKFILL_STATE_LOCK:
        if _OPENCLAW_WATCHDOG_BACKFILL_STATE:
            stale_cutoff = current - ttl_seconds
            stale_keys = [
                candidate_key
                for candidate_key, payload in _OPENCLAW_WATCHDOG_BACKFILL_STATE.items()
                if float(payload.get("updatedAtMono") or 0.0) < stale_cutoff
            ]
            for stale_key in stale_keys:
                _OPENCLAW_WATCHDOG_BACKFILL_STATE.pop(stale_key, None)
        payload = _OPENCLAW_WATCHDOG_BACKFILL_STATE.get(key)
        if not payload:
            return True
        next_allowed = float(payload.get("nextAllowedAtMono") or 0.0)
        return current >= next_allowed


def _openclaw_watchdog_backfill_record_result(
    session_key: str,
    *,
    changed: bool,
    failed: bool,
    ...
    with _OPENCLAW_WATCHDOG_BACKFILL_STATE_LOCK:
        if changed:
            _OPENCLAW_WATCHDOG_BACKFILL_STATE.pop(key, None)
            return
        payload = _OPENCLAW_WATCHDOG_BACKFILL_STATE.get(key) or {}
        streak = int(payload.get("streak") or 0)
        if failed:
            streak = min(16, streak + 2)
        else:
            streak = min(16, streak + 1)
        delay = _openclaw_watchdog_backfill_delay_seconds(streak)
        payload["streak"] = streak
        payload["nextAllowedAtMono"] = current + delay
        payload["updatedAtMono"] = current
        _OPENCLAW_WATCHDOG_BACKFILL_STATE[key] = payload
        if len(_OPENCLAW_WATCHDOG_BACKFILL_STATE) > 4096:
            oldest_key = min(
                _OPENCLAW_WATCHDOG_BACKFILL_STATE.items(),
                key=lambda item: float(item[1].get("updatedAtMono") or 0.0),
            )[0]
            _OPENCLAW_WATCHDOG_BACKFILL_STATE.pop(oldest_key, None)
    ...
    def _run(self) -> None:
        while True:
            ...
            try:
                ...
                _log_background_worker_exception("clawboard-openclaw-watchdog")
                try:
                    self.schedule(
                        session_key=base_key,
                        request_id=request_id,
        ...
        *,
        ...
        try:
            with get_session() as session:
                ...
                # Match exact session key, thread variant, and known board topic patterns.
                topic_id = _parse_board_session_key(base_key)
                query = select(LogEntry.id)
                if DATABASE_URL.startswith("sqlite"):
                    conditions = [
        ...
        if _openclaw_gateway_history_sync_enabled():
            if _openclaw_watchdog_backfill_should_run(base_key):
                try:
                    ...
                    _openclaw_watchdog_backfill_record_result(base_key, changed=changed, failed=False)
                    if changed:
                        with get_session() as session:
                            if _openclaw_watchdog_has_terminal_system_log(session, request_id):
                                return None
                    ...
                    _openclaw_watchdog_backfill_record_result(base_key, changed=False, failed=True)
...
def _shutdown_openclaw_assistant_log_watchdog() -> None:
    global _OPENCLAW_ASSISTANT_LOG_WATCHDOG
    with _OPENCLAW_ASSISTANT_LOG_WATCHDOG_LOCK:
        watchdog = _OPENCLAW_ASSISTANT_LOG_WATCHDOG
        _OPENCLAW_ASSISTANT_LOG_WATCHDOG = None
...
_OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF_LOCK = threading.Lock()
_OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF: dict[str, dict[str, Any]] = {}
...
def _is_gateway_transport_error(message: str) -> bool:
    text = str(message or "").strip().lower()
    if not text:
        return False
    if _is_gateway_timeout_error(text):
        return True
    tokens = [
        "connect failed",
...
def _openclaw_gateway_history_sync_session_backoff_seconds(streak: int) -> float:
    base = _openclaw_gateway_history_sync_session_backoff_base_seconds()
    max_delay = float(_openclaw_gateway_history_sync_max_backoff_seconds())
    exponent = max(0, min(10, int(streak or 0) - 1))
    delay = base * float(2**exponent)
    return max(base, min(max_delay, delay))


def _openclaw_gateway_history_sync_prune_session_backoff_state(now_mono: float | None = None) -> None:
    current = time.monotonic() if now_mono is None else float(now_mono)
    ttl_seconds = _openclaw_gateway_history_sync_session_backoff_ttl_seconds()
    stale_cutoff = current - ttl_seconds
    with _OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF_LOCK:
        stale_keys = [
            session_key
            for session_key, payload in _OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF.items()
            if float(payload.get("updatedAtMono") or 0.0) < stale_cutoff
        ]
        for session_key in stale_keys:
            _OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF.pop(session_key, None)


def _openclaw_gateway_history_sync_should_defer_session(
    session_key: str,
    *,
    now_mono: float | None = None,
) -> bool:
    ...
    current = time.monotonic() if now_mono is None else float(now_mono)
    with _OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF_LOCK:
        payload = _OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF.get(key)
        if not payload:
            return False
        next_allowed = float(payload.get("nextAllowedAtMono") or 0.0)
        return current < next_allowed
...
def _openclaw_gateway_history_sync_mark_session_success(session_key: str) -> None:
    key = str(session_key or "").strip()
    if not key:
        return
    with _OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF_LOCK:
        _OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF.pop(key, None)


def _openclaw_gateway_history_sync_mark_session_failure(
    session_key: str,
    *,
    error_text: str,
    now_mono: float | None = None,
    ...
    current = time.monotonic() if now_mono is None else float(now_mono)
    with _OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF_LOCK:
        payload = _OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF.get(key) or {}
        streak = min(16, int(payload.get("streak") or 0) + 1)
        payload["streak"] = streak
        payload["nextAllowedAtMono"] = current + _openclaw_gateway_history_sync_session_backoff_seconds(streak)
        payload["lastError"] = _clip(str(error_text or ""), 400)
        payload["updatedAtMono"] = current
        _OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF[key] = payload
        if len(_OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF) > 4096:
            oldest_key = min(
                _OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF.items(),
                key=lambda item: float(item[1].get("updatedAtMono") or 0.0),
            )[0]
            _OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF.pop(oldest_key, None)


def _openclaw_gateway_history_sync_backoff_state_size() -> int:
    with _OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF_LOCK:
        return len(_OPENCLAW_HISTORY_SYNC_SESSION_BACKOFF)
...
def _run_gateway_history_rpc_sync(method: str, params: dict[str, Any], *, scopes: list[str] | None = None) -> Any:
    ...
    transport_retry_attempts = _openclaw_gateway_history_sync_transport_retry_attempts()
    transport_retry_base = _openclaw_gateway_history_sync_transport_retry_base_seconds()

    last_exc: Exception | None = None
    last_error_text = ""
    for index, (attempt_token, device_override) in enumerate(attempts):
        for transport_attempt in range(max(1, transport_retry_attempts)):
            try:
                ...
                has_next_transport_attempt = transport_attempt + 1 < max(1, transport_retry_attempts)
                if has_next_transport_attempt and _is_gateway_transport_error(last_error_text):
                    delay = min(
                        float(_openclaw_gateway_history_sync_max_backoff_seconds()),
                        float(transport_retry_base) * float(2**transport_attempt),
                    )
        ...
        if _is_gateway_device_auth_error(last_error_text) or _is_gateway_scope_error(last_error_text):
            continue
        if _is_gateway_transport_error(last_error_text):
            continue
        break
...
def _openclaw_history_scope_metadata(message: dict[str, Any], session_key: str) -> dict[str, Any]:
    ...
    topic_id = _parse_board_session_key(session_key)
...
def _ingest_openclaw_history_messages(*, session_key: str, messages: list[Any], since_ms: int) -> tuple[int, int]:
    ...
    session_board_topic_id = _parse_board_session_key(session_key)
    is_board_session = bool(session_board_topic_id)
    with get_session() as session:
...
def _openclaw_gateway_history_subagent_seed_sessions(*, limit: int, lookback_seconds: int) -> list[str]:
    """Seed child subagent sessions from recent sessions_spawn lineage logs."""
    if limit <= 0:
        return []
    cutoff = (datetime.now(timezone.utc) - timedelta(seconds=max(300, int(lookback_seconds or 300)))).isoformat(
        timespec="milliseconds"
    ).replace("+00:00", "Z")
    fetch_limit = max(limit, min(6000, limit * 60))
    try:
        with get_session() as session:
            rows = (
                session.exec(
                    select(LogEntry)
                    .where(LogEntry.createdAt >= cutoff)
                    .where(LogEntry.type.in_(["action", "system", "conversation"]))
                    .order_by(LogEntry.createdAt.desc())
                    .limit(fetch_limit)
                ).all()
            )
    ...
    for row in rows:
        ...
        source_channel = str(source.get("channel") or "").strip().lower()
        source_session = str(source.get("sessionKey") or "").strip()
        if source_channel and source_channel not in {"openclaw", "clawboard", "webchat"}:
            if not source_session or not _openclaw_history_session_key_likely_gateway(source_session):
                continue
        lower_combined = _combined_log_text(
            _safe_log_attr_text(row, "content"),
            _safe_log_attr_text(row, "summary"),
            _safe_log_attr_text(row, "raw"),
        ).lower()
        if "sessions_spawn" not in lower_combined and "subagent" not in lower_combined:
            continue
        for child_session in _orchestration_extract_child_session_keys(row):
            child_base = _base_session_key(child_session) or child_session
            if not child_base or child_base in seen:
                continue
            if not _openclaw_history_session_key_likely_gateway(child_base):
                continue
            seen.add(child_base)
            result.append(child_base)
            if len(result) >= limit:
                return result
    return result
...
def _sync_openclaw_gateway_history_once() -> dict[str, Any]:
    ...
    subagent_seed_keys = _openclaw_gateway_history_subagent_seed_sessions(
        limit=_openclaw_gateway_history_sync_subagent_seed_limit(),
        lookback_seconds=_openclaw_gateway_history_sync_subagent_seed_lookback_seconds(),
    )
    ...
    for session_key in unresolved_seed_keys:
        ...
        candidates_by_key.setdefault(session_key, 0)
    for session_key in subagent_seed_keys:
        # sessions_spawn lineage seeds recover delegated subagent transcripts when sessions.list is degraded/disabled.
        candidates_by_key.setdefault(session_key, 0)
    ...
    for seed_group in [unresolved_seed_keys, subagent_seed_keys, cursor_seed_keys, log_seed_keys]:
    ...
    failed_sessions = 0
    deferred_sessions = 0
    deferred_by_backoff = 0
    timeout_recovered_sessions = 0
    failure_samples: list[str] = []
    _openclaw_gateway_history_sync_prune_session_backoff_state(cycle_started)

    for idx, (session_key, updated_at_ms) in enumerate(candidates):
        now_mono = time.monotonic()
        if (now_mono - cycle_started) >= cycle_budget_seconds:
            deferred_sessions += max(0, len(candidates) - idx)
            break
        if _openclaw_gateway_history_sync_should_defer_session(session_key, now_mono=now_mono):
            deferred_sessions += 1
            deferred_by_backoff += 1
            continue
        ...
        try:
            ...
            if not isinstance(messages, list):
                failed_sessions += 1
                _openclaw_gateway_history_sync_mark_session_failure(
                    session_key,
        ...
        if not isinstance(messages, list):
            continue
        _openclaw_gateway_history_sync_mark_session_success(session_key)
    ...
    return {
        ...
        "deferredByBackoff": int(deferred_by_backoff),
...
def _openclaw_gateway_history_sync_worker() -> None:
    ...
    while not _BACKGROUND_STOP_EVENT.is_set():
        ...
        if _background_sleep(max(5.0, float(sleep_seconds))):
            break
...
def _resolve_board_send_target(
    ...
    *,
    ...
    if explicit_new_topic:
        ...
        decision_source = "direct_selected_topic"
    else:
...
def resolve_board_send(payload: OpenClawResolveBoardSendRequest):
    ...
    with get_session() as session:
        ...
        session_key = f"clawboard:topic:{topic.id}"
        return {
...
def openclaw_chat(payload: OpenClawChatRequest, _background_tasks: BackgroundTasks):
    ...
    try:
        ...
        with get_session() as session:
            ...
            if topic_id:
                source_meta["boardScopeTopicId"] = topic_id
                source_meta["boardScopeKind"] = "topic"
                source_meta["boardScopeLock"] = True
...
def openclaw_chat_cancel(payload: OpenClawChatCancelRequest):
    ...
    the dispatch workers will not retry them. The cancel scope fans out to linked
    subagent sessions discovered via requestId + recent board-scope activity.
    """
    session_key = payload.sessionKey.strip()
    ...
    cancel_session_keys: list[str] = [session_key]
    cancel_request_ids: list[str] = [request_id_filter] if request_id_filter else []

    # --- Step 1: resolve linked sessions + cancel pending/retry/processing dispatch queue rows ---
    cancelled_count = 0
    now_val = now_iso()
    with get_session() as db:
        try:
            resolved_sessions, resolved_request_ids = _collect_openclaw_cancel_related_sessions(
                db,
                session_key=session_key,
                request_id_filter=request_id_filter,
            )
            if resolved_sessions:
                cancel_session_keys = resolved_sessions
            if resolved_request_ids:
                cancel_request_ids = resolved_request_ids
        except Exception:
            # Fallback to primary session-only cancellation when linkage resolution fails.
            base_key = _base_session_key(session_key)
            cancel_session_keys = [key for key in [session_key, base_key] if key]
            cancel_request_ids = [request_id_filter] if request_id_filter else []

        q = select(OpenClawChatDispatchQueue).where(
            OpenClawChatDispatchQueue.status.in_(["pending", "retry", "processing"])
        )
        if cancel_session_keys:
            q = q.where(OpenClawChatDispatchQueue.sessionKey.in_(cancel_session_keys))
        rows = db.exec(q).all()
        for row in rows:
            row_request = str(getattr(row, "requestId", "") or "").strip()
            if request_id_filter and not _openclaw_request_ids_equivalent(row_request, request_id_filter):
                continue
            row.status = "failed"
            row.lastError = "user_cancelled"
            row.completedAt = now_val
            row.claimedAt = None
            row.nextAttemptAt = now_val
    ...
    if not abort_request_id and len(cancel_request_ids) == 1:
        abort_request_id = cancel_request_ids[0]
    ...
    gateway_abort_count = 0
    for target_session_key in cancel_session_keys:
        ...
        try:
            ...
            gateway_abort_count += 1
        except Exception:
            continue

    return {
        "aborted": gateway_abort_count > 0,
...
def _openclaw_agent_env_suffix(agent_id: str) -> str | None:
    safe = re.sub(r"[^A-Z0-9]+", "_", str(agent_id or "").strip().upper()).strip("_")
    return safe or None


def _openclaw_model_config_values(raw: Any) -> list[str]:
    if isinstance(raw, str):
        value = raw.strip()
...
def _split_model_ref(raw: Any) -> tuple[str, str] | None:
...
def _openclaw_model_supports_image(cfg: dict[str, Any], raw_model_ref: Any) -> bool:
    parsed = _split_model_ref(raw_model_ref)
    if not parsed:
        return False
    provider, model_id = parsed
    providers_cfg = cfg.get("models") if isinstance(cfg.get("models"), dict) else {}
    providers = providers_cfg.get("providers") if isinstance(providers_cfg.get("providers"), dict) else {}
    provider_entry = None
    for key, value in providers.items():
        if str(key or "").strip().lower() == provider.lower() and isinstance(value, dict):
            provider_entry = value
            break
    if not isinstance(provider_entry, dict):
        return False
    catalog = provider_entry.get("models")
    ...
    for entry in catalog:
        ...
        if entry_id.lower() != model_id.lower():
            continue
        inputs = entry.get("input")
        if not isinstance(inputs, list):
            return False
        return any(str(item or "").strip().lower() == "image" for item in inputs)
    return False


def _openclaw_chat_attachments_require_vision(attachments: list[dict[str, Any]] | None) -> bool:
    if not attachments:
        return False
    for attachment in attachments:
        mime_type = _normalize_mime_type(str(attachment.get("mimeType") or ""))
        if mime_type in ATTACHMENT_IMAGE_MIME_TYPES or mime_type == "application/pdf":
            return True
    return False


def _resolve_openclaw_attachment_model_ref(agent_id: str, attachments: list[dict[str, Any]] | None) -> str | None:
    if not _openclaw_chat_attachments_require_vision(attachments):
        return None
    suffix = _openclaw_agent_env_suffix(agent_id)
    env_candidates: list[str] = []
    if suffix:
        specific = str(os.getenv(f"CLAWBOARD_OPENCLAW_ATTACHMENT_MODEL_{suffix}") or "").strip()
        if specific:
            env_candidates.append(specific)
    generic = str(os.getenv("CLAWBOARD_OPENCLAW_ATTACHMENT_MODEL") or "").strip()
    if generic:
        env_candidates.append(generic)
    if env_candidates:
    ...
    agents_cfg = cfg.get("agents") if isinstance(cfg.get("agents"), dict) else {}
    defaults_cfg = agents_cfg.get("defaults") if isinstance(agents_cfg.get("defaults"), dict) else {}
    image_model_values = _openclaw_model_config_values(defaults_cfg.get("imageModel"))
    if image_model_values:
        return image_model_values[0]

    for candidate in _openclaw_model_config_values(defaults_cfg.get("model")):
        if _openclaw_model_supports_image(cfg, candidate):
            return candidate
    return None


def _safe_openclaw_attachment_path_token(raw: Any, *, fallback: str) -> str:
    text = str(raw or "").strip()
...
def _openclaw_attachment_stage_relative_dir(*, session_key: str, request_id: str) -> str:
    session_token = _safe_openclaw_attachment_path_token(session_key, fallback="session")
    request_token = _safe_openclaw_attachment_path_token(request_id, fallback="request")
    return "/".join([".", ".openclaw", "clawboard-attachments", session_token, request_token])


def _stage_openclaw_chat_attachments_for_agents(
    *,
    request_id: str,
    session_key: str,
    attachments: list[dict[str, Any]],
) -> dict[str, Any]:
    relative_dir = _openclaw_attachment_stage_relative_dir(session_key=session_key, request_id=request_id)
    relative_parts = [part for part in relative_dir.split("/") if part and part != "."]
    files_manifest: list[dict[str, Any]] = []
    for attachment in attachments:
        filename = _sanitize_attachment_filename(str(attachment.get("fileName") or "attachment"))
        files_manifest.append(
            {
                "id": str(attachment.get("id") or "").strip(),
                "fileName": filename,
                "mimeType": _normalize_mime_type(str(attachment.get("mimeType") or "application/octet-stream")),
                "sizeBytes": int(attachment.get("sizeBytes") or 0),
                "relativePath": f"{relative_dir}/{filename}",
            }
        )
    ...
    staged_workspaces: list[dict[str, str]] = []
    seen_workspaces: set[str] = set()
    for row in _resolve_openclaw_agent_workspaces():
        workspace_dir = _normalize_fs_path(row.get("workspaceDir"))
        if not workspace_dir:
            continue
        workspace_key = workspace_dir.lower() if os.name == "nt" else workspace_dir
        if workspace_key in seen_workspaces:
            continue
        seen_workspaces.add(workspace_key)
        target_dir = Path(workspace_dir, *relative_parts)
        target_dir.mkdir(parents=True, exist_ok=True)
        for attachment in attachments:
            filename = _sanitize_attachment_filename(str(attachment.get("fileName") or "attachment"))
            data_bytes = attachment.get("dataBytes")
            if not isinstance(data_bytes, (bytes, bytearray)):
                raise RuntimeError(f"attachment staging missing bytes ({attachment.get('id') or filename})")
            target_path = target_dir / filename
            target_path.write_bytes(bytes(data_bytes))
        (target_dir / ".manifest.json").write_text(manifest_text, encoding="utf-8")
        staged_workspaces.append(
            {
                "agentId": str(row.get("agentId") or "").strip() or "main",
                "workspaceDir": workspace_dir,
            }
        )

    if not staged_workspaces:
        raise RuntimeError("unable to stage attachments into any OpenClaw workspace")

    return {
        "relativeDir": relative_dir,
        "manifestRelativePath": f"{relative_dir}/.manifest.json",
        "files": files_manifest,
        "workspaces": staged_workspaces,
    }


def _build_openclaw_attachment_handoff_note(stage_info: dict[str, Any]) -> str:
    files = stage_info.get("files") if isinstance(stage_info.get("files"), list) else []
    file_summary = ", ".join(
    ...
    relative_paths = [
        str(item.get("relativePath") or "").strip()
        for item in files
    ...
    lines = [
        ...
        "If you delegate this work, include the same exact file path (or staged directory) in the subagent task so the child can inspect the files from its own workspace.",
    ]
    if file_summary:
        lines.insert(3, f"Files: {file_summary}")
    if relative_paths:
        insert_at = 4 if file_summary else 3
        lines[insert_at:insert_at] = ["Exact workspace-relative file paths:", *[f"- {path}" for path in relative_paths]]
    return "\n".join(lines)


def _ensure_openclaw_attachment_model_for_dispatch(
    *,
    session_key: str,
    agent_id: str,
    ...
    if not _openclaw_chat_attachments_require_vision(attachments):
        return None
    model_ref = _resolve_openclaw_attachment_model_ref(agent_id, attachments)
    if not model_ref:
        raise RuntimeError(
            "image attachments require a vision-capable OpenClaw model; set agents.defaults.imageModel.primary "
            "or CLAWBOARD_OPENCLAW_ATTACHMENT_MODEL"
        )
    asyncio.run(
        gateway_rpc(
            "sessions.patch",
            ...
            timeout_seconds=min(8.0, _openclaw_chat_send_rpc_timeout_seconds()),
        )
    )
    return model_ref
...
# --- dispatch_synthetic: moved to .dispatch module ---
...
def admin_start_fresh_replay(
    ...
    Default mode (`reclassify`) keeps existing topic links and only re-queues unassigned/failed
    conversation logs for classification. `fresh` clears derived topics first.
    """
    timestamp = now_iso()
    with get_session() as session:
        instance = session.get(InstanceConfig, 1)
        ...
        if payload.replayMode == "fresh":
            ...
            # need allocation (missing topic) or previously failed classification.
            session.exec(
                text(
                    f"""
                    UPDATE logentry
...
# Task API routes removed — topology flattened to Space→Topic.
...
def list_pending_conversations_for_classifier(
    ...
    with get_session() as session:
        ...
        rows = session.exec(query).all()
        if allowed_space_ids is not None:
            topic_by_id = _load_related_maps_for_logs(session, rows)
            rows = [row for row in rows if _log_matches_allowed_spaces(row, allowed_space_ids, topic_by_id)]
        if offset > 0:
            rows = rows[offset:]
        if limit >= 0:
            rows = rows[:limit]
...
def replay_classifier_bundle(payload: ClassifierReplayRequest):
    ...
    with get_session() as session:
        ...
        topic_id = _parse_board_session_key(base_session_key)
        if not topic_id:
...
def list_logs(
    ...
    with get_session() as session:
        ...
        while len(rows) < limit:
            ...
            if not chunk:
                break
            topic_by_id = _load_related_maps_for_logs(session, chunk)
            for row in chunk:
                if not _log_matches_allowed_spaces(row, allowed_space_ids, topic_by_id):
                    continue
                if skip > 0:
                    skip -= 1
                    continue
...
def get_log_chat_counts(
    ...
    """Return aggregate topic chat entry counts without loading full log payloads."""
    with get_session() as session:
        allowed_space_ids = _resolve_allowed_space_ids(
            session,
            source_space_id=spaceId,
        ...
        topic_query = (
            select(LogEntry.topicId, func.count(LogEntry.id))
            .where(LogEntry.topicId.is_not(None))
        ...
        for row in topic_rows:
            ...
            if not tid:
                continue
            topic_counts[tid] = max(0, count_value)
...
def patch_log(log_id: str, payload: LogPatch = Body(...)):
    ...
    with get_session() as session:
        ...
        # Keep log space in sync with routed topic ownership.
        if entry.topicId:
        ...
        # Keep source-level board scope metadata aligned with the effective routing.
        source_map = entry.source.copy() if isinstance(entry.source, dict) else {}
        if source_map:
            canonical_space = _normalize_space_id(entry.spaceId)
            canonical_topic = str(entry.topicId or "").strip()
            if canonical_space:
                source_map["boardScopeSpaceId"] = canonical_space
            if canonical_topic:
                source_map["boardScopeTopicId"] = canonical_topic
        ...
        if (
            ...
            and (
                bool(request_route_event.get("promotion"))
                or str(request_route_event.get("previousTopicId") or "").strip() != str(entry.topicId or "").strip()
            )
        ):
            try:
                board_route_retro_scoped_rows = _retro_scope_request_route_rows(
...
def purge_log_forward(log_id: str):
    ...
    with get_session() as session:
        ...
        if not topic_id:
            raise HTTPException(status_code=400, detail="Anchor log has no topicId; purge forward requires a scoped entry")

        # Scope the query to the anchor's topic thread.
        q_filter = (LogEntry.topicId == topic_id,)
...
def _build_changes_payload(
    ...
    *,
    ...
    if not since:
        ...
        logs = session.exec(log_query).all()
        if allowed_space_ids is not None:
            topic_by_id_for_logs = _load_related_maps_for_logs(
                session,
                logs,
            ...
            logs = [
                row
                for row in logs
                if _log_matches_allowed_spaces(row, allowed_space_ids, topic_by_id_for_logs)
            ]
    else:
    ...
    openclaw_typing, openclaw_thread_work = _build_openclaw_live_signal_snapshots(session)
    cursor = ""
    for collection in (spaces, topics, logs, drafts):
...
def list_changes(
    ...
    """Return topics/logs changed since timestamp (ISO).
    ...
    *,
    ...
    if allowed_space_ids is not None:
        logs = [
            entry
            for entry in logs
            if _log_matches_allowed_spaces(entry, allowed_space_ids, all_topic_by_id)
        ]
...
def clawgraph(
    ...
    """Build and return an entity-relationship graph from topics/logs."""
    with get_session() as session:
        allowed_space_ids = _resolve_allowed_space_ids(
            session,
            source_space_id=spaceId,
    ...
    *,
    ...
    return "|".join(
        [
            ...
            _sanitize_log_text(semantic_query or "").lower(),
...
def _search_impl(
    ...
    *,
    ...
    window_log_count = 0
    long_query_dense_expansion_mode = "disabled"
    long_query_dense_expansion_candidate_count = 0
    long_query_dense_expansion_added = 0
    long_query_dense_expansion_added_ids: list[str] = []
    if allowed_space_ids is None or allowed_space_values:
        ...
        dense_query_for_expansion = _sanitize_log_text(str(semantic_query or query))
        dense_query_tokens = _search_query_tokens(dense_query_for_expansion.lower()) if dense_query_for_expansion else set()
        enable_long_query_dense_expansion = (
            SEARCH_LONG_QUERY_DENSE_EXPANSION_ENABLED
            and SEARCH_LONG_QUERY_DENSE_EXPANSION_MAX_LOGS > 0
            and allow_deep_content_scan
            and bool(dense_query_for_expansion)
            and (
                len(dense_query_for_expansion) >= SEARCH_LONG_QUERY_DENSE_EXPANSION_MIN_CHARS
                or len(dense_query_tokens) >= SEARCH_LONG_QUERY_DENSE_EXPANSION_MIN_TOKENS
            )
        )
        if enable_long_query_dense_expansion:
            long_query_dense_expansion_mode = "candidate"
            dense_log_limit = max(
                int(effective_limit_logs),
                min(
                    int(SEARCH_LONG_QUERY_DENSE_EXPANSION_MAX_LOGS),
                    max(int(effective_limit_logs), int(effective_limit_logs) * 3),
                ),
            )
            dense_candidates = dense_candidate_ids(
                dense_query_for_expansion,
                topic_limit=max(32, int(effective_limit_topics)),
                log_limit=max(80, int(dense_log_limit)),
            )
            dense_candidate_ids_raw = list(dense_candidates.get("logs") or [])
            long_query_dense_expansion_candidate_count = len(dense_candidate_ids_raw)
            dense_candidate_ids_clean: list[str] = []
            seen_dense_ids: set[str] = set()
            for raw_id in dense_candidate_ids_raw:
                parsed_id = str(raw_id or "").strip()
                if not parsed_id or parsed_id in seen_dense_ids:
                    continue
                seen_dense_ids.add(parsed_id)
                dense_candidate_ids_clean.append(parsed_id)
                if len(dense_candidate_ids_clean) >= dense_log_limit:
                    break

            if dense_candidate_ids_clean:
                existing_ids = {str(getattr(entry, "id", "") or "").strip() for entry in logs if getattr(entry, "id", None)}
                missing_dense_ids = [item_id for item_id in dense_candidate_ids_clean if item_id not in existing_ids]
                if missing_dense_ids:
                    dense_logs_query = select(
                        LogEntry.id,
                        LogEntry.topicId,
                        LogEntry.relatedLogId,
                        LogEntry.idempotencyKey,
                        ...
                        LogEntry.source,
                        LogEntry.spaceId,
                    ).where(LogEntry.id.in_(missing_dense_ids))
                    if topic_id:
                        dense_logs_query = dense_logs_query.where(LogEntry.topicId == topic_id)
                    if not include_pending:
                        dense_logs_query = dense_logs_query.where(LogEntry.classificationStatus == "classified")
                    if allowed_space_ids is not None:
                        dense_logs_query = dense_logs_query.where(LogEntry.spaceId.in_(allowed_space_values))

                    dense_rows = session.exec(dense_logs_query).all()
                    for row in dense_rows:
                        ...
                        existing_ids.add(parsed_id)
                        logs.append(parsed)
                        long_query_dense_expansion_added_ids.append(parsed_id)

                    long_query_dense_expansion_added = len(long_query_dense_expansion_added_ids)
                    if long_query_dense_expansion_added > 0:
                        long_query_dense_expansion_mode = str(dense_candidates.get("mode") or "applied")
                    else:
                        long_query_dense_expansion_mode = "no-new-rows"
                else:
                    long_query_dense_expansion_mode = "already-in-window"
            else:
                long_query_dense_expansion_mode = str(dense_candidates.get("mode") or "no-candidates")
    ...
    if preview_scan_cap > 0:
        ...
        if allow_deep_content_scan and long_query_dense_expansion_added_ids:
            dense_preview_cap = min(
                len(long_query_dense_expansion_added_ids),
                max(20, min(180, int(effective_limit_logs))),
            )
            preview_scan_ids = list(
                dict.fromkeys(long_query_dense_expansion_added_ids[:dense_preview_cap] + preview_scan_ids)
            )
            preview_scan_ids = preview_scan_ids[: max(preview_scan_cap, dense_preview_cap)]
    else:
        preview_scan_ids = []
    ...
    if not semantic_query and _is_low_signal_context_query(normalized_query):
        ...
        board_topic_id = _parse_board_session_key(session_key or "")
        if board_topic_id:
    ...
    def _append_hint(
        ...
        *,
        ...
        bucket = topic_query_hints.setdefault(bucket_key, [])
    ...
    if hint_query_tokens:
        for entry in logs:
            if not _log_allowed_for_semantic_search(entry):
                continue
            topic_key = str(entry.topicId or "").strip() or None
            _append_hint(topic_key, str(entry.summary or ""))
            preview = content_preview_by_log_id.get(str(entry.id), "")
            if preview:
                _append_hint(topic_key, preview)
        for item in routing_items[-6:]:
            ...
            _append_hint(topic_id_hint, topic_name_hint)
            _append_hint(topic_id_hint, anchor_hint)
    ...
    return {
        ...
        "searchMeta": {
            ...
            "longQueryDenseExpansionEnabled": bool(SEARCH_LONG_QUERY_DENSE_EXPANSION_ENABLED),
            "longQueryDenseExpansionMode": str(long_query_dense_expansion_mode),
            "longQueryDenseExpansionCandidates": int(long_query_dense_expansion_candidate_count),
            "longQueryDenseExpansionAdded": int(long_query_dense_expansion_added),
...
def search(
    ...
    semanticQuery: str | None = Query(
        default=None,
        description="Optional long-form semantic query hint used for dense/hybrid ranking.",
    ),
