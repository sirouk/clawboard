...
class AdminEndpointTests(unittest.TestCase):
    ...
    def test_start_fresh_replay_defaults_to_non_destructive_reclassify(self):
        ...
        with get_session() as session:
            ...
            topic_scoped = session.get(LogEntry, "log-2")
            self.assertIsNotNone(topic_scoped)
            self.assertEqual(topic_scoped.topicId, "topic-1")
            self.assertIsNone(topic_scoped.taskId)
            self.assertEqual(topic_scoped.classificationStatus, "pending")
            self.assertEqual(topic_scoped.classificationAttempts, 0)
            self.assertIsNone(topic_scoped.classificationError)
