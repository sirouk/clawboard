...
- Do not send another status-only update if nothing materially changed and the last visible status for that topic was less than 5 minutes ago.
...
6. For each "doing" topic with a `"session:<key>"` tag whose `session_status` lookup is missing, failed, or clearly terminal without a relayed result: the delegation was lost. Call `clawboard_get_topic(topicId)` to recover the topic context, re-spawn with `sessions_spawn(agentId, originalTask)`, update the topic tags with the new session key (plus explicit compatibility task tags only when a legacy task row is in scope), and `cron.add` a new follow-up at `+1m`.
