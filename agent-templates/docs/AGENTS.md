...
- For Clawboard repo docs, resolve the repo root from the configured OpenClaw workspaces instead of assuming a home-directory path. Prefer, in order: an explicit path from the task, the current working tree if it is the Clawboard repo, or a `projects/clawboard` checkout under the main workspace described by the installation config. Do not assume `OPENCLAW_HOME` is set. Do not wander into unrelated OpenClaw docs trees unless the task explicitly asks for OpenClaw docs.
