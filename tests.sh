...
run_with_retry "backend critical chat regressions (dedupe + stop timing)" docker compose run --rm -T \
  ...
  sh -lc 'PIP_DISABLE_PIP_VERSION_CHECK=1 pip install --quiet --root-user-action=ignore httpx && cd /app/tests && python -m unittest -v \
    ...
    test_openclaw_chat_watchdog_and_ingest.OpenClawChatAndIngestTests.test_chat_020j1_board_probe_fallback_respects_global_disable \
    test_openclaw_chat_watchdog_and_ingest.OpenClawChatAndIngestTests.test_chat_020j2_orchestration_follow_up_dispatch_uses_occhat_prefixed_request_id \
