...
export default function register(api) {
    ...
    const contextMode = (rawConfig.contextMode && ["auto", "cheap", "full", "patient"].includes(rawConfig.contextMode)
        ...
        : "auto");
    const contextFallbackMode = (rawConfig.contextFallbackMode && ["auto", "cheap", "full", "patient"].includes(rawConfig.contextFallbackMode)
    ...
    const contextFetchTimeoutMs = typeof rawConfig.contextFetchTimeoutMs === "number" && Number.isFinite(rawConfig.contextFetchTimeoutMs)
        ...
        : 1200;
    const contextTotalBudgetMs = typeof rawConfig.contextTotalBudgetMs === "number" && Number.isFinite(rawConfig.contextTotalBudgetMs)
    ...
    function coerceTopics(data) {
        return Array.isArray(data) ? data : [];
    }
    function coerceTasks(data) {
        return Array.isArray(data) ? data : [];
    }
    ...
    async function retrieveContext(query, sessionKey, upstream) {
        ...
        const taskBuckets = await Promise.all(topics.map(async (topic) => {
        ...
        const fallbackNotes = relatedLogId.length > 0
            ? await listLogs({
                type: "note",
                relatedLogId,
                limit: 120,
    ...
    const rememberIncoming = (key) => {
        ...
        setTimeout(() => recentIncoming.delete(key), 30_000)?.unref?.();
    };
