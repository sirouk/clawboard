...
export default function register(api: OpenClawPluginApi) {
  ...
  const rawContextMode =
    typeof rawConfig.contextMode === "string" && rawConfig.contextMode.trim()
  ...
  const effectiveContextMode: ContextMode = isContextMode(normalizedContextMode) ? normalizedContextMode : "auto";
  ...
  const contextFetchRetries =
    typeof rawConfig.contextFetchRetries === "number" && Number.isFinite(rawConfig.contextFetchRetries)
  ...
  const contextCacheTtlMs =
    typeof rawConfig.contextCacheTtlMs === "number" && Number.isFinite(rawConfig.contextCacheTtlMs)
  ...
  const contextCacheMaxEntries =
    typeof rawConfig.contextCacheMaxEntries === "number" && Number.isFinite(rawConfig.contextCacheMaxEntries)
  ...
  const contextUseCacheOnFailure =
  ...
  const SOURCE_EXTRA_CANDIDATE_KEY_RE =
    /(id|_id|session|channel|provider|role|type|status|phase|kind|call|run|attempt|retry|synthetic)$/i;
  ...
  function normalizeSourceExtraKey(rawKey: string) {
  ...
  function sourceScalarValue(value: unknown): string | number | boolean | undefined {
  ...
  function mergeSourceExtras(target: Record<string, unknown>, extra: Record<string, unknown> | undefined) {
    if (!extra) return;
    for (const [rawKey, rawValue] of Object.entries(extra)) {
      const key = normalizeSourceExtraKey(rawKey);
      if (!key) continue;
      if (SOURCE_META_RESERVED_KEYS.has(key)) continue;
      if (Object.prototype.hasOwnProperty.call(target, key)) continue;
      if (SOURCE_EXTRA_KEY_BLOCK_RE.test(key)) continue;
  ...
  function hookSourceHints(params: {
    ...
    if (messageRole) extra.messageRole = messageRole.toLowerCase();
    if (messageType) extra.messageType = messageType;
    ...
    for (const record of dynamicRecords) {
      ...
      for (const [rawKey, rawValue] of Object.entries(record)) {
        ...
        if (Object.prototype.hasOwnProperty.call(extra, normalizedKey)) continue;
        if (SOURCE_META_RESERVED_KEYS.has(normalizedKey)) continue;
        if (SOURCE_EXTRA_KEY_BLOCK_RE.test(normalizedKey)) continue;
        if (!SOURCE_EXTRA_CANDIDATE_KEY_RE.test(normalizedKey)) continue;
        const scalar = sourceScalarValue(rawValue);
        if (scalar === undefined) continue;
  ...
  function buildSourceMeta(params: {
    ...
    if (requestId) source.requestId = requestId;
  ...
  function hasSpecificSessionAnchor(
    ...
    for (const candidate of candidates) {
      ...
      if (lowered === "channel:openclaw" || lowered === "channel:clawboard") continue;
      return true;
    }
    return false;
  }
  ...
  function extractSpawnedSubagentSessionKeys(value: unknown) {
    ...
    const CHILD_KEY_FIELDS = ["childSessionKey", "child_session_key"] as const;
    const CHILD_LIST_FIELDS = ["childSessionKeys", "child_session_keys"] as const;

    const record = (candidate: unknown) => {
      const key = normalizeId(typeof candidate === "string" ? candidate : undefined);
      if (!key) return;
      if (!parseSubagentSession(key)) return;
      out.add(key);
    };
    ...
    const visit = (node: unknown, depth: number) => {
      ...
      for (const field of CHILD_LIST_FIELDS) {
        ...
        for (const item of list) record(item);
      }
  ...
  function rememberSpawnedSubagentBoardScope(childSessionKey: string, scope: BoardScope) {
    const key = normalizeId(childSessionKey);
    if (!key) return;
    if (!parseSubagentSession(key)) return;
    const inheritedScope: BoardScope = {
      ...scope,
      inherited: true,
      updatedAt: nowMs(),
    };
  ...
  function uniqueSessionCandidates(values: Array<string | undefined | null>) {
    const out = new Set<string>();
  ...
  function warnBaseUrlFallback(fromBaseUrl: string, toBaseUrl: string, err: unknown) {
    const now = nowMs();
    const reason = formatSendError(err);
    const sig = `${fromBaseUrl}->${toBaseUrl}:${reason}`;
    suppressedBaseFallbackWarns += 1;
    const shouldLog =
      lastBaseFallbackWarnAt === 0 ||
    ...
    api.logger.warn(
      `[clawboard-logger] clawboard API fallback: ${fromBaseUrl} -> ${toBaseUrl} (reason: ${reason})${suffix}`,
    );
  ...
  function buildCandidateUrl(base: string, pathname: string, query?: Record<string, QueryParamValue>) {
  ...
  function toolErrorText(result: { data?: unknown } | undefined) {
  ...
  function isToolTransientFailure(result: { ok: boolean; status: number; data: unknown }) {
    if (result.ok) return false;
    if (result.status === 429 || result.status === 502 || result.status === 503 || result.status === 504) return true;
  ...
  function toolDataRecord(value: unknown) {
    if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
    return value as Record<string, unknown>;
  }

  function toolDataArray(value: unknown) {
      ...
      (ctxTool: unknown) => {
        ...
        tools.push({
          ...
          async execute(_toolCallId: string, params: Record<string, unknown>) {
            ...
            if (!retry.ok) return toolJsonResult(retry);
    ...
    if (contextUseCacheOnFailure && contextCacheTtlMs > 0) {
      for (const currentMode of modes) {
        ...
        if (debug) {
          ...
          api.logger.warn(
            `[clawboard-logger] context retrieval failed (${lastError || "unknown"}); using cached context mode=${cached.mode} ageMs=${ageMs}`,
          );
        }
        return cached.block;
      }
  ...
  beforeAgentStartApi.on("before_agent_start", async (event: PluginHookBeforeAgentStartEvent, ctx: PluginHookAgentContext) => {
    ...
    if (shouldIgnoreSessionKey(sessionKeyForContext, IGNORE_SESSION_PREFIXES)) return;
    ...
    const isSubagentScaffoldPrompt =
      cleanInput &&
    ...
    const retrievalQuery =
      cleanInput && cleanInput.trim().length > 0 && !isSubagentScaffoldPrompt
        ? clip(cleanInput, 320)
  ...
  const openclawRequestBySession = new Map<string, { requestId: string; ts: number }>();
  type ToolScopeMemoryEntry = {
    ts: number;
    sessionKey?: string;
    requestId?: string;
  ...
  const pruneOpenclawRequestMap = (ts: number, forceTrim = false) => {
    ...
    if (!forceTrim && openclawRequestBySession.size <= OPENCLAW_REQUEST_ID_MAX_ENTRIES) return;
    if (openclawRequestBySession.size <= OPENCLAW_REQUEST_ID_MAX_ENTRIES) return;
  ...
  const rememberOpenclawRequestId = (sessionKey: string | undefined, requestId: unknown) => {
  ...
  const recentOpenclawRequestId = (sessionKey: string | undefined) => {
    const ts = nowMs();
  ...
  const toolScopeNameKey = (toolName: unknown) => {
  ...
  const rememberToolScopeForName = (
    toolName: unknown,
    value: { sessionKey?: string; requestId?: string; routing?: RoutingScope }
  ) => {
  ...
  const recentToolScopeForRun = (runId: unknown) => {
    const key = normalizeId(typeof runId === "string" ? runId : undefined);
    if (!key) return undefined;
    const ts = nowMs();
  ...
  const resolveSessionKey = (meta: { sessionKey?: string } | undefined, ctx2: PluginHookContextBase) => {
    ...
    if (parseSubagentSession(ctxSession)) return ctxSession;
  ...
  api.on("message_received", async (event: PluginHookMessageReceivedEvent, ctx: PluginHookMessageContext) => {
    ...
    const messageId =
      typeof meta?.messageId === "string" && meta.messageId.trim().length > 0
        ? meta.messageId
        : typeof (event as unknown as { messageId?: unknown }).messageId === "string"
    ...
    if (
      shouldSuppressNonSemanticConversation(cleanRaw, {
        sessionKey: effectiveSessionKey ?? ctx.sessionKey,
        channelId: ctx.channelId,
      })
    ) {
  ...
  api.on("before_message_write", async (event: PluginHookBeforeMessageWriteEvent, ctx: PluginHookMessageContext) => {
    const createdAt = new Date().toISOString();
    const eventRecord = asRecord(event);
    const messageRecord = asRecord((event as { message?: unknown }).message);
    if (!messageRecord) return;
    const meta = normalizeEventMeta(
      asRecord((event as { metadata?: unknown }).metadata),
      (event as { sessionKey?: unknown }).sessionKey,
    );
    const effectiveSessionKey = resolveSessionKey(meta as { sessionKey?: string } | undefined, ctx);
    if (shouldIgnoreSessionKey(effectiveSessionKey ?? ctx?.sessionKey, IGNORE_SESSION_PREFIXES)) return;
    ...
    if (messageRole === "assistant" || messageRole === "user") return;
    ...
    if (recentTranscriptWrites.has(dedupeKey)) return;
  ...
  api.on("tool_result_persist", async (event: PluginHookToolResultPersistEvent, ctx: PluginHookToolContext) => {
    const createdAt = new Date().toISOString();
    const eventRecord = asRecord(event);
    const messageRecord = asRecord((event as { message?: unknown }).message);
    const meta = normalizeEventMeta(
    ...
    if (recentToolResultPersist.has(dedupeKey)) return;
  ...
  api.on("after_tool_call", async (event: PluginHookAfterToolCallEvent, ctx: PluginHookToolContext) => {
    ...
    const routing =
      hasToolRoutingAnchor(routingResolved) || !remembered?.routing ? routingResolved : remembered.routing;
    requestId = await resolveOpenclawRequestIdForBoardScope({
      requestId,
      sessionKey: effectiveSessionKey ?? ctx.sessionKey,
      boardScope: routing.boardScope,
  ...
  api.on("agent_end", async (event: PluginHookAgentEndEvent, ctx: PluginHookAgentContext) => {
    ...
    if (!inferredSessionKey) {
      ...
      for (let idx = startIdx; idx < messages.length; idx += 1) {
        ...
        if (
          shouldSuppressNonSemanticConversation(cleanedContent, {
            sessionKey: inferredSessionKey,
  ...
  if (configuredExtraHooks.length > 0) {
    ...
    const genericHookApi = api as unknown as {
      on: (event: string, handler: (event: HookEvent, ctx: PluginHookContextBase) => unknown) => void;
    };

    for (const hookName of configuredExtraHooks) {
      if (!hookName || knownHooks.has(hookName)) continue;
      genericHookApi.on(hookName, async (event: HookEvent, ctx: PluginHookContextBase) => {
        const createdAt = new Date().toISOString();
        const eventRecord = asRecord(event);
        const messageRecord = asRecord((eventRecord as { message?: unknown } | undefined)?.message);
        const meta = normalizeEventMeta(
          asRecord((eventRecord as { metadata?: unknown } | undefined)?.metadata),
          (eventRecord as { sessionKey?: unknown } | undefined)?.sessionKey,
        );
        const effectiveSessionKey = resolveSessionKey(meta as { sessionKey?: string } | undefined, ctx);
        if (shouldIgnoreSessionKey(effectiveSessionKey ?? ctx?.sessionKey, IGNORE_SESSION_PREFIXES)) return;
